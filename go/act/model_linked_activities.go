/*
OPERA Cloud Activity API

APIs to cater for Sales Activity functionality in OPERA Cloud. <br /><br /> Activities provide you with an account management tool for managing daily tasks such as appointments, sales calls, contact follow-up, and so on.<br /><br /> Compatible with OPERA Cloud release 26.2.0.0.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2026 Oracle and/or its affiliates.</p>

API version: 26.2.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package act

import (
	"encoding/json"
)

// checks if the LinkedActivities type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &LinkedActivities{}

// LinkedActivities struct for LinkedActivities
type LinkedActivities struct {
	LinkedActivityDetails *LinkedActivitiesType `json:"linkedActivityDetails,omitempty"`
	Links []InstanceLink `json:"links,omitempty"`
	// Used in conjunction with the Success element to define a business error.
	Warnings []WarningType `json:"warnings,omitempty"`
}

// NewLinkedActivities instantiates a new LinkedActivities object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewLinkedActivities() *LinkedActivities {
	this := LinkedActivities{}
	return &this
}

// NewLinkedActivitiesWithDefaults instantiates a new LinkedActivities object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewLinkedActivitiesWithDefaults() *LinkedActivities {
	this := LinkedActivities{}
	return &this
}

// GetLinkedActivityDetails returns the LinkedActivityDetails field value if set, zero value otherwise.
func (o *LinkedActivities) GetLinkedActivityDetails() LinkedActivitiesType {
	if o == nil || IsNil(o.LinkedActivityDetails) {
		var ret LinkedActivitiesType
		return ret
	}
	return *o.LinkedActivityDetails
}

// GetLinkedActivityDetailsOk returns a tuple with the LinkedActivityDetails field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *LinkedActivities) GetLinkedActivityDetailsOk() (*LinkedActivitiesType, bool) {
	if o == nil || IsNil(o.LinkedActivityDetails) {
		return nil, false
	}
	return o.LinkedActivityDetails, true
}

// HasLinkedActivityDetails returns a boolean if a field has been set.
func (o *LinkedActivities) HasLinkedActivityDetails() bool {
	if o != nil && !IsNil(o.LinkedActivityDetails) {
		return true
	}

	return false
}

// SetLinkedActivityDetails gets a reference to the given LinkedActivitiesType and assigns it to the LinkedActivityDetails field.
func (o *LinkedActivities) SetLinkedActivityDetails(v LinkedActivitiesType) {
	o.LinkedActivityDetails = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *LinkedActivities) GetLinks() []InstanceLink {
	if o == nil || IsNil(o.Links) {
		var ret []InstanceLink
		return ret
	}
	return o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *LinkedActivities) GetLinksOk() ([]InstanceLink, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *LinkedActivities) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given []InstanceLink and assigns it to the Links field.
func (o *LinkedActivities) SetLinks(v []InstanceLink) {
	o.Links = v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *LinkedActivities) GetWarnings() []WarningType {
	if o == nil || IsNil(o.Warnings) {
		var ret []WarningType
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *LinkedActivities) GetWarningsOk() ([]WarningType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *LinkedActivities) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []WarningType and assigns it to the Warnings field.
func (o *LinkedActivities) SetWarnings(v []WarningType) {
	o.Warnings = v
}

func (o LinkedActivities) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o LinkedActivities) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.LinkedActivityDetails) {
		toSerialize["linkedActivityDetails"] = o.LinkedActivityDetails
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableLinkedActivities struct {
	value *LinkedActivities
	isSet bool
}

func (v NullableLinkedActivities) Get() *LinkedActivities {
	return v.value
}

func (v *NullableLinkedActivities) Set(val *LinkedActivities) {
	v.value = val
	v.isSet = true
}

func (v NullableLinkedActivities) IsSet() bool {
	return v.isSet
}

func (v *NullableLinkedActivities) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableLinkedActivities(val *LinkedActivities) *NullableLinkedActivities {
	return &NullableLinkedActivities{value: val, isSet: true}
}

func (v NullableLinkedActivities) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableLinkedActivities) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



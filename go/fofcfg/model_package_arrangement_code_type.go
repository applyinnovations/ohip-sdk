/*
OPERA Cloud Front Desk Configuration API

APIs to cater for Front Desk Configuration in OPERA Cloud. Here you can find operations to get, post, put and delete front desk codes such as commission codes, transaction groups, codes & subgroups, articles, payment methods and credit card types.<br /><br />Compatible with OPERA Cloud release 26.2.0.0.<br /><br /><p>This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020,2023 Oracle and/or its affiliates.</p><br /><br /> Compatible with OPERA Cloud release 20.99.99.99.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2026 Oracle and/or its affiliates.</p>

API version: 26.2.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fofcfg

import (
	"encoding/json"
)

// checks if the PackageArrangementCodeType type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PackageArrangementCodeType{}

// PackageArrangementCodeType Generic Type to specify unique/primary id for the code. mostly used for Hotel level configuration codes removal request.
type PackageArrangementCodeType struct {
	// Hotel where the code is configured.
	HotelId *string `json:"hotelId,omitempty"`
	// Configuration code.
	Code *string `json:"code,omitempty"`
	// Primay key of arrangement codes
	ArrangementId *float32 `json:"arrangementId,omitempty"`
	Description *TranslationTextType2000 `json:"description,omitempty"`
	// information about the tax type code linked to the folio arrangment Code.
	TaxTypeCode *string `json:"taxTypeCode,omitempty"`
	// specify whether the arrangement postings are classed as revenue.
	Revenue *bool `json:"revenue,omitempty"`
	// Represents whether the package arrangement code is inactive or not
	Inactive *bool `json:"inactive,omitempty"`
	ManagedBy *ManagedByOptions `json:"managedBy,omitempty"`
}

// NewPackageArrangementCodeType instantiates a new PackageArrangementCodeType object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPackageArrangementCodeType() *PackageArrangementCodeType {
	this := PackageArrangementCodeType{}
	return &this
}

// NewPackageArrangementCodeTypeWithDefaults instantiates a new PackageArrangementCodeType object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPackageArrangementCodeTypeWithDefaults() *PackageArrangementCodeType {
	this := PackageArrangementCodeType{}
	return &this
}

// GetHotelId returns the HotelId field value if set, zero value otherwise.
func (o *PackageArrangementCodeType) GetHotelId() string {
	if o == nil || IsNil(o.HotelId) {
		var ret string
		return ret
	}
	return *o.HotelId
}

// GetHotelIdOk returns a tuple with the HotelId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PackageArrangementCodeType) GetHotelIdOk() (*string, bool) {
	if o == nil || IsNil(o.HotelId) {
		return nil, false
	}
	return o.HotelId, true
}

// HasHotelId returns a boolean if a field has been set.
func (o *PackageArrangementCodeType) HasHotelId() bool {
	if o != nil && !IsNil(o.HotelId) {
		return true
	}

	return false
}

// SetHotelId gets a reference to the given string and assigns it to the HotelId field.
func (o *PackageArrangementCodeType) SetHotelId(v string) {
	o.HotelId = &v
}

// GetCode returns the Code field value if set, zero value otherwise.
func (o *PackageArrangementCodeType) GetCode() string {
	if o == nil || IsNil(o.Code) {
		var ret string
		return ret
	}
	return *o.Code
}

// GetCodeOk returns a tuple with the Code field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PackageArrangementCodeType) GetCodeOk() (*string, bool) {
	if o == nil || IsNil(o.Code) {
		return nil, false
	}
	return o.Code, true
}

// HasCode returns a boolean if a field has been set.
func (o *PackageArrangementCodeType) HasCode() bool {
	if o != nil && !IsNil(o.Code) {
		return true
	}

	return false
}

// SetCode gets a reference to the given string and assigns it to the Code field.
func (o *PackageArrangementCodeType) SetCode(v string) {
	o.Code = &v
}

// GetArrangementId returns the ArrangementId field value if set, zero value otherwise.
func (o *PackageArrangementCodeType) GetArrangementId() float32 {
	if o == nil || IsNil(o.ArrangementId) {
		var ret float32
		return ret
	}
	return *o.ArrangementId
}

// GetArrangementIdOk returns a tuple with the ArrangementId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PackageArrangementCodeType) GetArrangementIdOk() (*float32, bool) {
	if o == nil || IsNil(o.ArrangementId) {
		return nil, false
	}
	return o.ArrangementId, true
}

// HasArrangementId returns a boolean if a field has been set.
func (o *PackageArrangementCodeType) HasArrangementId() bool {
	if o != nil && !IsNil(o.ArrangementId) {
		return true
	}

	return false
}

// SetArrangementId gets a reference to the given float32 and assigns it to the ArrangementId field.
func (o *PackageArrangementCodeType) SetArrangementId(v float32) {
	o.ArrangementId = &v
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *PackageArrangementCodeType) GetDescription() TranslationTextType2000 {
	if o == nil || IsNil(o.Description) {
		var ret TranslationTextType2000
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PackageArrangementCodeType) GetDescriptionOk() (*TranslationTextType2000, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *PackageArrangementCodeType) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given TranslationTextType2000 and assigns it to the Description field.
func (o *PackageArrangementCodeType) SetDescription(v TranslationTextType2000) {
	o.Description = &v
}

// GetTaxTypeCode returns the TaxTypeCode field value if set, zero value otherwise.
func (o *PackageArrangementCodeType) GetTaxTypeCode() string {
	if o == nil || IsNil(o.TaxTypeCode) {
		var ret string
		return ret
	}
	return *o.TaxTypeCode
}

// GetTaxTypeCodeOk returns a tuple with the TaxTypeCode field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PackageArrangementCodeType) GetTaxTypeCodeOk() (*string, bool) {
	if o == nil || IsNil(o.TaxTypeCode) {
		return nil, false
	}
	return o.TaxTypeCode, true
}

// HasTaxTypeCode returns a boolean if a field has been set.
func (o *PackageArrangementCodeType) HasTaxTypeCode() bool {
	if o != nil && !IsNil(o.TaxTypeCode) {
		return true
	}

	return false
}

// SetTaxTypeCode gets a reference to the given string and assigns it to the TaxTypeCode field.
func (o *PackageArrangementCodeType) SetTaxTypeCode(v string) {
	o.TaxTypeCode = &v
}

// GetRevenue returns the Revenue field value if set, zero value otherwise.
func (o *PackageArrangementCodeType) GetRevenue() bool {
	if o == nil || IsNil(o.Revenue) {
		var ret bool
		return ret
	}
	return *o.Revenue
}

// GetRevenueOk returns a tuple with the Revenue field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PackageArrangementCodeType) GetRevenueOk() (*bool, bool) {
	if o == nil || IsNil(o.Revenue) {
		return nil, false
	}
	return o.Revenue, true
}

// HasRevenue returns a boolean if a field has been set.
func (o *PackageArrangementCodeType) HasRevenue() bool {
	if o != nil && !IsNil(o.Revenue) {
		return true
	}

	return false
}

// SetRevenue gets a reference to the given bool and assigns it to the Revenue field.
func (o *PackageArrangementCodeType) SetRevenue(v bool) {
	o.Revenue = &v
}

// GetInactive returns the Inactive field value if set, zero value otherwise.
func (o *PackageArrangementCodeType) GetInactive() bool {
	if o == nil || IsNil(o.Inactive) {
		var ret bool
		return ret
	}
	return *o.Inactive
}

// GetInactiveOk returns a tuple with the Inactive field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PackageArrangementCodeType) GetInactiveOk() (*bool, bool) {
	if o == nil || IsNil(o.Inactive) {
		return nil, false
	}
	return o.Inactive, true
}

// HasInactive returns a boolean if a field has been set.
func (o *PackageArrangementCodeType) HasInactive() bool {
	if o != nil && !IsNil(o.Inactive) {
		return true
	}

	return false
}

// SetInactive gets a reference to the given bool and assigns it to the Inactive field.
func (o *PackageArrangementCodeType) SetInactive(v bool) {
	o.Inactive = &v
}

// GetManagedBy returns the ManagedBy field value if set, zero value otherwise.
func (o *PackageArrangementCodeType) GetManagedBy() ManagedByOptions {
	if o == nil || IsNil(o.ManagedBy) {
		var ret ManagedByOptions
		return ret
	}
	return *o.ManagedBy
}

// GetManagedByOk returns a tuple with the ManagedBy field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PackageArrangementCodeType) GetManagedByOk() (*ManagedByOptions, bool) {
	if o == nil || IsNil(o.ManagedBy) {
		return nil, false
	}
	return o.ManagedBy, true
}

// HasManagedBy returns a boolean if a field has been set.
func (o *PackageArrangementCodeType) HasManagedBy() bool {
	if o != nil && !IsNil(o.ManagedBy) {
		return true
	}

	return false
}

// SetManagedBy gets a reference to the given ManagedByOptions and assigns it to the ManagedBy field.
func (o *PackageArrangementCodeType) SetManagedBy(v ManagedByOptions) {
	o.ManagedBy = &v
}

func (o PackageArrangementCodeType) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PackageArrangementCodeType) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.HotelId) {
		toSerialize["hotelId"] = o.HotelId
	}
	if !IsNil(o.Code) {
		toSerialize["code"] = o.Code
	}
	if !IsNil(o.ArrangementId) {
		toSerialize["arrangementId"] = o.ArrangementId
	}
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	if !IsNil(o.TaxTypeCode) {
		toSerialize["taxTypeCode"] = o.TaxTypeCode
	}
	if !IsNil(o.Revenue) {
		toSerialize["revenue"] = o.Revenue
	}
	if !IsNil(o.Inactive) {
		toSerialize["inactive"] = o.Inactive
	}
	if !IsNil(o.ManagedBy) {
		toSerialize["managedBy"] = o.ManagedBy
	}
	return toSerialize, nil
}

type NullablePackageArrangementCodeType struct {
	value *PackageArrangementCodeType
	isSet bool
}

func (v NullablePackageArrangementCodeType) Get() *PackageArrangementCodeType {
	return v.value
}

func (v *NullablePackageArrangementCodeType) Set(val *PackageArrangementCodeType) {
	v.value = val
	v.isSet = true
}

func (v NullablePackageArrangementCodeType) IsSet() bool {
	return v.isSet
}

func (v *NullablePackageArrangementCodeType) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePackageArrangementCodeType(val *PackageArrangementCodeType) *NullablePackageArrangementCodeType {
	return &NullablePackageArrangementCodeType{value: val, isSet: true}
}

func (v NullablePackageArrangementCodeType) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePackageArrangementCodeType) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



/*
OPERA Cloud Front Desk Configuration API

APIs to cater for Front Desk Configuration in OPERA Cloud. Here you can find operations to get, post, put and delete front desk codes such as commission codes, transaction groups, codes & subgroups, articles, payment methods and credit card types.<br /><br />Compatible with OPERA Cloud release 26.2.0.0.<br /><br /><p>This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020,2023 Oracle and/or its affiliates.</p><br /><br /> Compatible with OPERA Cloud release 20.99.99.99.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2026 Oracle and/or its affiliates.</p>

API version: 26.2.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fofcfg

import (
	"encoding/json"
)

// checks if the FiscalFolioParameters type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &FiscalFolioParameters{}

// FiscalFolioParameters struct for FiscalFolioParameters
type FiscalFolioParameters struct {
	// Holds Fiscal Folio Parameter details.
	FiscalFolioParameters []FiscalFolioParameterType `json:"fiscalFolioParameters,omitempty"`
	Links []InstanceLink `json:"links,omitempty"`
	// Used in conjunction with the Success element to define a business error.
	Warnings []WarningType `json:"warnings,omitempty"`
}

// NewFiscalFolioParameters instantiates a new FiscalFolioParameters object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFiscalFolioParameters() *FiscalFolioParameters {
	this := FiscalFolioParameters{}
	return &this
}

// NewFiscalFolioParametersWithDefaults instantiates a new FiscalFolioParameters object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFiscalFolioParametersWithDefaults() *FiscalFolioParameters {
	this := FiscalFolioParameters{}
	return &this
}

// GetFiscalFolioParameters returns the FiscalFolioParameters field value if set, zero value otherwise.
func (o *FiscalFolioParameters) GetFiscalFolioParameters() []FiscalFolioParameterType {
	if o == nil || IsNil(o.FiscalFolioParameters) {
		var ret []FiscalFolioParameterType
		return ret
	}
	return o.FiscalFolioParameters
}

// GetFiscalFolioParametersOk returns a tuple with the FiscalFolioParameters field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FiscalFolioParameters) GetFiscalFolioParametersOk() ([]FiscalFolioParameterType, bool) {
	if o == nil || IsNil(o.FiscalFolioParameters) {
		return nil, false
	}
	return o.FiscalFolioParameters, true
}

// HasFiscalFolioParameters returns a boolean if a field has been set.
func (o *FiscalFolioParameters) HasFiscalFolioParameters() bool {
	if o != nil && !IsNil(o.FiscalFolioParameters) {
		return true
	}

	return false
}

// SetFiscalFolioParameters gets a reference to the given []FiscalFolioParameterType and assigns it to the FiscalFolioParameters field.
func (o *FiscalFolioParameters) SetFiscalFolioParameters(v []FiscalFolioParameterType) {
	o.FiscalFolioParameters = v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *FiscalFolioParameters) GetLinks() []InstanceLink {
	if o == nil || IsNil(o.Links) {
		var ret []InstanceLink
		return ret
	}
	return o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FiscalFolioParameters) GetLinksOk() ([]InstanceLink, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *FiscalFolioParameters) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given []InstanceLink and assigns it to the Links field.
func (o *FiscalFolioParameters) SetLinks(v []InstanceLink) {
	o.Links = v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *FiscalFolioParameters) GetWarnings() []WarningType {
	if o == nil || IsNil(o.Warnings) {
		var ret []WarningType
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FiscalFolioParameters) GetWarningsOk() ([]WarningType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *FiscalFolioParameters) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []WarningType and assigns it to the Warnings field.
func (o *FiscalFolioParameters) SetWarnings(v []WarningType) {
	o.Warnings = v
}

func (o FiscalFolioParameters) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o FiscalFolioParameters) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.FiscalFolioParameters) {
		toSerialize["fiscalFolioParameters"] = o.FiscalFolioParameters
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableFiscalFolioParameters struct {
	value *FiscalFolioParameters
	isSet bool
}

func (v NullableFiscalFolioParameters) Get() *FiscalFolioParameters {
	return v.value
}

func (v *NullableFiscalFolioParameters) Set(val *FiscalFolioParameters) {
	v.value = val
	v.isSet = true
}

func (v NullableFiscalFolioParameters) IsSet() bool {
	return v.isSet
}

func (v *NullableFiscalFolioParameters) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFiscalFolioParameters(val *FiscalFolioParameters) *NullableFiscalFolioParameters {
	return &NullableFiscalFolioParameters{value: val, isSet: true}
}

func (v NullableFiscalFolioParameters) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFiscalFolioParameters) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



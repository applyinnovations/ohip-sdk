/*
OPERA Cloud Front Desk Configuration API

APIs to cater for Front Desk Configuration in OPERA Cloud. Here you can find operations to get, post, put and delete front desk codes such as commission codes, transaction groups, codes & subgroups, articles, payment methods and credit card types.<br /><br />Compatible with OPERA Cloud release 26.2.0.0.<br /><br /><p>This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020,2023 Oracle and/or its affiliates.</p><br /><br /> Compatible with OPERA Cloud release 20.99.99.99.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2026 Oracle and/or its affiliates.</p>

API version: 26.2.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fofcfg

import (
	"encoding/json"
)

// checks if the CopyTrackItTypes type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CopyTrackItTypes{}

// CopyTrackItTypes struct for CopyTrackItTypes
type CopyTrackItTypes struct {
	// List of Track it Types to be copied.
	TrackItTypes []CopyConfigurationCodeType `json:"trackItTypes,omitempty"`
	Links []InstanceLink `json:"links,omitempty"`
	// Used in conjunction with the Success element to define a business error.
	Warnings []WarningType `json:"warnings,omitempty"`
}

// NewCopyTrackItTypes instantiates a new CopyTrackItTypes object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCopyTrackItTypes() *CopyTrackItTypes {
	this := CopyTrackItTypes{}
	return &this
}

// NewCopyTrackItTypesWithDefaults instantiates a new CopyTrackItTypes object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCopyTrackItTypesWithDefaults() *CopyTrackItTypes {
	this := CopyTrackItTypes{}
	return &this
}

// GetTrackItTypes returns the TrackItTypes field value if set, zero value otherwise.
func (o *CopyTrackItTypes) GetTrackItTypes() []CopyConfigurationCodeType {
	if o == nil || IsNil(o.TrackItTypes) {
		var ret []CopyConfigurationCodeType
		return ret
	}
	return o.TrackItTypes
}

// GetTrackItTypesOk returns a tuple with the TrackItTypes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CopyTrackItTypes) GetTrackItTypesOk() ([]CopyConfigurationCodeType, bool) {
	if o == nil || IsNil(o.TrackItTypes) {
		return nil, false
	}
	return o.TrackItTypes, true
}

// HasTrackItTypes returns a boolean if a field has been set.
func (o *CopyTrackItTypes) HasTrackItTypes() bool {
	if o != nil && !IsNil(o.TrackItTypes) {
		return true
	}

	return false
}

// SetTrackItTypes gets a reference to the given []CopyConfigurationCodeType and assigns it to the TrackItTypes field.
func (o *CopyTrackItTypes) SetTrackItTypes(v []CopyConfigurationCodeType) {
	o.TrackItTypes = v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *CopyTrackItTypes) GetLinks() []InstanceLink {
	if o == nil || IsNil(o.Links) {
		var ret []InstanceLink
		return ret
	}
	return o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CopyTrackItTypes) GetLinksOk() ([]InstanceLink, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *CopyTrackItTypes) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given []InstanceLink and assigns it to the Links field.
func (o *CopyTrackItTypes) SetLinks(v []InstanceLink) {
	o.Links = v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *CopyTrackItTypes) GetWarnings() []WarningType {
	if o == nil || IsNil(o.Warnings) {
		var ret []WarningType
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CopyTrackItTypes) GetWarningsOk() ([]WarningType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *CopyTrackItTypes) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []WarningType and assigns it to the Warnings field.
func (o *CopyTrackItTypes) SetWarnings(v []WarningType) {
	o.Warnings = v
}

func (o CopyTrackItTypes) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CopyTrackItTypes) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.TrackItTypes) {
		toSerialize["trackItTypes"] = o.TrackItTypes
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableCopyTrackItTypes struct {
	value *CopyTrackItTypes
	isSet bool
}

func (v NullableCopyTrackItTypes) Get() *CopyTrackItTypes {
	return v.value
}

func (v *NullableCopyTrackItTypes) Set(val *CopyTrackItTypes) {
	v.value = val
	v.isSet = true
}

func (v NullableCopyTrackItTypes) IsSet() bool {
	return v.isSet
}

func (v *NullableCopyTrackItTypes) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCopyTrackItTypes(val *CopyTrackItTypes) *NullableCopyTrackItTypes {
	return &NullableCopyTrackItTypes{value: val, isSet: true}
}

func (v NullableCopyTrackItTypes) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCopyTrackItTypes) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



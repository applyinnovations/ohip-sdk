/*
OPERA Cloud Block Reservation Asynchronous API

APIs to cater Block Reservation related asynchronous functionality in OPERA.<br /><br /> Compatible with OPERA Cloud release 26.2.0.0.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2026 Oracle and/or its affiliates.</p>

API version: 26.2.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package blkasync

import (
	"encoding/json"
)

// checks if the BlockChangesByDateTimeSummary type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &BlockChangesByDateTimeSummary{}

// BlockChangesByDateTimeSummary struct for BlockChangesByDateTimeSummary
type BlockChangesByDateTimeSummary struct {
	// Collection of Business Block changes by date and time.
	BlockList []BlockChangesByDateTimeSummaryType `json:"blockList,omitempty"`
	// Indicates the index of the next applicable set(page).
	Offset *int32 `json:"offset,omitempty"`
	// Indicates number of records the API can return as per the API request limit sent. A maximum of 200 records can be only returned at a time.
	Limit *int32 `json:"limit,omitempty"`
	// Indicates number of records the API has returned actually as per the API request criteria.
	Count *int32 `json:"count,omitempty"`
	// Indicates whether there are more records available to be returned as per the API request criteria or not.
	HasMore *bool `json:"hasMore,omitempty"`
	// Indicates total number of records available that can be returned as per the API request criteria.
	TotalResults *int32 `json:"totalResults,omitempty"`
}

// NewBlockChangesByDateTimeSummary instantiates a new BlockChangesByDateTimeSummary object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewBlockChangesByDateTimeSummary() *BlockChangesByDateTimeSummary {
	this := BlockChangesByDateTimeSummary{}
	return &this
}

// NewBlockChangesByDateTimeSummaryWithDefaults instantiates a new BlockChangesByDateTimeSummary object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewBlockChangesByDateTimeSummaryWithDefaults() *BlockChangesByDateTimeSummary {
	this := BlockChangesByDateTimeSummary{}
	return &this
}

// GetBlockList returns the BlockList field value if set, zero value otherwise.
func (o *BlockChangesByDateTimeSummary) GetBlockList() []BlockChangesByDateTimeSummaryType {
	if o == nil || IsNil(o.BlockList) {
		var ret []BlockChangesByDateTimeSummaryType
		return ret
	}
	return o.BlockList
}

// GetBlockListOk returns a tuple with the BlockList field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BlockChangesByDateTimeSummary) GetBlockListOk() ([]BlockChangesByDateTimeSummaryType, bool) {
	if o == nil || IsNil(o.BlockList) {
		return nil, false
	}
	return o.BlockList, true
}

// HasBlockList returns a boolean if a field has been set.
func (o *BlockChangesByDateTimeSummary) HasBlockList() bool {
	if o != nil && !IsNil(o.BlockList) {
		return true
	}

	return false
}

// SetBlockList gets a reference to the given []BlockChangesByDateTimeSummaryType and assigns it to the BlockList field.
func (o *BlockChangesByDateTimeSummary) SetBlockList(v []BlockChangesByDateTimeSummaryType) {
	o.BlockList = v
}

// GetOffset returns the Offset field value if set, zero value otherwise.
func (o *BlockChangesByDateTimeSummary) GetOffset() int32 {
	if o == nil || IsNil(o.Offset) {
		var ret int32
		return ret
	}
	return *o.Offset
}

// GetOffsetOk returns a tuple with the Offset field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BlockChangesByDateTimeSummary) GetOffsetOk() (*int32, bool) {
	if o == nil || IsNil(o.Offset) {
		return nil, false
	}
	return o.Offset, true
}

// HasOffset returns a boolean if a field has been set.
func (o *BlockChangesByDateTimeSummary) HasOffset() bool {
	if o != nil && !IsNil(o.Offset) {
		return true
	}

	return false
}

// SetOffset gets a reference to the given int32 and assigns it to the Offset field.
func (o *BlockChangesByDateTimeSummary) SetOffset(v int32) {
	o.Offset = &v
}

// GetLimit returns the Limit field value if set, zero value otherwise.
func (o *BlockChangesByDateTimeSummary) GetLimit() int32 {
	if o == nil || IsNil(o.Limit) {
		var ret int32
		return ret
	}
	return *o.Limit
}

// GetLimitOk returns a tuple with the Limit field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BlockChangesByDateTimeSummary) GetLimitOk() (*int32, bool) {
	if o == nil || IsNil(o.Limit) {
		return nil, false
	}
	return o.Limit, true
}

// HasLimit returns a boolean if a field has been set.
func (o *BlockChangesByDateTimeSummary) HasLimit() bool {
	if o != nil && !IsNil(o.Limit) {
		return true
	}

	return false
}

// SetLimit gets a reference to the given int32 and assigns it to the Limit field.
func (o *BlockChangesByDateTimeSummary) SetLimit(v int32) {
	o.Limit = &v
}

// GetCount returns the Count field value if set, zero value otherwise.
func (o *BlockChangesByDateTimeSummary) GetCount() int32 {
	if o == nil || IsNil(o.Count) {
		var ret int32
		return ret
	}
	return *o.Count
}

// GetCountOk returns a tuple with the Count field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BlockChangesByDateTimeSummary) GetCountOk() (*int32, bool) {
	if o == nil || IsNil(o.Count) {
		return nil, false
	}
	return o.Count, true
}

// HasCount returns a boolean if a field has been set.
func (o *BlockChangesByDateTimeSummary) HasCount() bool {
	if o != nil && !IsNil(o.Count) {
		return true
	}

	return false
}

// SetCount gets a reference to the given int32 and assigns it to the Count field.
func (o *BlockChangesByDateTimeSummary) SetCount(v int32) {
	o.Count = &v
}

// GetHasMore returns the HasMore field value if set, zero value otherwise.
func (o *BlockChangesByDateTimeSummary) GetHasMore() bool {
	if o == nil || IsNil(o.HasMore) {
		var ret bool
		return ret
	}
	return *o.HasMore
}

// GetHasMoreOk returns a tuple with the HasMore field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BlockChangesByDateTimeSummary) GetHasMoreOk() (*bool, bool) {
	if o == nil || IsNil(o.HasMore) {
		return nil, false
	}
	return o.HasMore, true
}

// HasHasMore returns a boolean if a field has been set.
func (o *BlockChangesByDateTimeSummary) HasHasMore() bool {
	if o != nil && !IsNil(o.HasMore) {
		return true
	}

	return false
}

// SetHasMore gets a reference to the given bool and assigns it to the HasMore field.
func (o *BlockChangesByDateTimeSummary) SetHasMore(v bool) {
	o.HasMore = &v
}

// GetTotalResults returns the TotalResults field value if set, zero value otherwise.
func (o *BlockChangesByDateTimeSummary) GetTotalResults() int32 {
	if o == nil || IsNil(o.TotalResults) {
		var ret int32
		return ret
	}
	return *o.TotalResults
}

// GetTotalResultsOk returns a tuple with the TotalResults field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BlockChangesByDateTimeSummary) GetTotalResultsOk() (*int32, bool) {
	if o == nil || IsNil(o.TotalResults) {
		return nil, false
	}
	return o.TotalResults, true
}

// HasTotalResults returns a boolean if a field has been set.
func (o *BlockChangesByDateTimeSummary) HasTotalResults() bool {
	if o != nil && !IsNil(o.TotalResults) {
		return true
	}

	return false
}

// SetTotalResults gets a reference to the given int32 and assigns it to the TotalResults field.
func (o *BlockChangesByDateTimeSummary) SetTotalResults(v int32) {
	o.TotalResults = &v
}

func (o BlockChangesByDateTimeSummary) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o BlockChangesByDateTimeSummary) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.BlockList) {
		toSerialize["blockList"] = o.BlockList
	}
	if !IsNil(o.Offset) {
		toSerialize["offset"] = o.Offset
	}
	if !IsNil(o.Limit) {
		toSerialize["limit"] = o.Limit
	}
	if !IsNil(o.Count) {
		toSerialize["count"] = o.Count
	}
	if !IsNil(o.HasMore) {
		toSerialize["hasMore"] = o.HasMore
	}
	if !IsNil(o.TotalResults) {
		toSerialize["totalResults"] = o.TotalResults
	}
	return toSerialize, nil
}

type NullableBlockChangesByDateTimeSummary struct {
	value *BlockChangesByDateTimeSummary
	isSet bool
}

func (v NullableBlockChangesByDateTimeSummary) Get() *BlockChangesByDateTimeSummary {
	return v.value
}

func (v *NullableBlockChangesByDateTimeSummary) Set(val *BlockChangesByDateTimeSummary) {
	v.value = val
	v.isSet = true
}

func (v NullableBlockChangesByDateTimeSummary) IsSet() bool {
	return v.isSet
}

func (v *NullableBlockChangesByDateTimeSummary) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableBlockChangesByDateTimeSummary(val *BlockChangesByDateTimeSummary) *NullableBlockChangesByDateTimeSummary {
	return &NullableBlockChangesByDateTimeSummary{value: val, isSet: true}
}

func (v NullableBlockChangesByDateTimeSummary) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableBlockChangesByDateTimeSummary) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



/*
OPERA Cloud Block Reservation Asynchronous API

APIs to cater Block Reservation related asynchronous functionality in OPERA.<br /><br /> Compatible with OPERA Cloud release 26.2.0.0.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2026 Oracle and/or its affiliates.</p>

API version: 26.2.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package blkasync

import (
	"encoding/json"
)

// checks if the BlockAllocationWash type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &BlockAllocationWash{}

// BlockAllocationWash Request object for block wash operation.
type BlockAllocationWash struct {
	BlockAllocationWashType *BlockAllocationWashType `json:"blockAllocationWashType,omitempty"`
	Links []InstanceLink `json:"links,omitempty"`
	// Used in conjunction with the Success element to define a business error.
	Warnings []WarningType `json:"warnings,omitempty"`
}

// NewBlockAllocationWash instantiates a new BlockAllocationWash object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewBlockAllocationWash() *BlockAllocationWash {
	this := BlockAllocationWash{}
	return &this
}

// NewBlockAllocationWashWithDefaults instantiates a new BlockAllocationWash object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewBlockAllocationWashWithDefaults() *BlockAllocationWash {
	this := BlockAllocationWash{}
	return &this
}

// GetBlockAllocationWashType returns the BlockAllocationWashType field value if set, zero value otherwise.
func (o *BlockAllocationWash) GetBlockAllocationWashType() BlockAllocationWashType {
	if o == nil || IsNil(o.BlockAllocationWashType) {
		var ret BlockAllocationWashType
		return ret
	}
	return *o.BlockAllocationWashType
}

// GetBlockAllocationWashTypeOk returns a tuple with the BlockAllocationWashType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BlockAllocationWash) GetBlockAllocationWashTypeOk() (*BlockAllocationWashType, bool) {
	if o == nil || IsNil(o.BlockAllocationWashType) {
		return nil, false
	}
	return o.BlockAllocationWashType, true
}

// HasBlockAllocationWashType returns a boolean if a field has been set.
func (o *BlockAllocationWash) HasBlockAllocationWashType() bool {
	if o != nil && !IsNil(o.BlockAllocationWashType) {
		return true
	}

	return false
}

// SetBlockAllocationWashType gets a reference to the given BlockAllocationWashType and assigns it to the BlockAllocationWashType field.
func (o *BlockAllocationWash) SetBlockAllocationWashType(v BlockAllocationWashType) {
	o.BlockAllocationWashType = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *BlockAllocationWash) GetLinks() []InstanceLink {
	if o == nil || IsNil(o.Links) {
		var ret []InstanceLink
		return ret
	}
	return o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BlockAllocationWash) GetLinksOk() ([]InstanceLink, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *BlockAllocationWash) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given []InstanceLink and assigns it to the Links field.
func (o *BlockAllocationWash) SetLinks(v []InstanceLink) {
	o.Links = v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *BlockAllocationWash) GetWarnings() []WarningType {
	if o == nil || IsNil(o.Warnings) {
		var ret []WarningType
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BlockAllocationWash) GetWarningsOk() ([]WarningType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *BlockAllocationWash) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []WarningType and assigns it to the Warnings field.
func (o *BlockAllocationWash) SetWarnings(v []WarningType) {
	o.Warnings = v
}

func (o BlockAllocationWash) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o BlockAllocationWash) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.BlockAllocationWashType) {
		toSerialize["blockAllocationWashType"] = o.BlockAllocationWashType
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableBlockAllocationWash struct {
	value *BlockAllocationWash
	isSet bool
}

func (v NullableBlockAllocationWash) Get() *BlockAllocationWash {
	return v.value
}

func (v *NullableBlockAllocationWash) Set(val *BlockAllocationWash) {
	v.value = val
	v.isSet = true
}

func (v NullableBlockAllocationWash) IsSet() bool {
	return v.isSet
}

func (v *NullableBlockAllocationWash) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableBlockAllocationWash(val *BlockAllocationWash) *NullableBlockAllocationWash {
	return &NullableBlockAllocationWash{value: val, isSet: true}
}

func (v NullableBlockAllocationWash) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableBlockAllocationWash) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



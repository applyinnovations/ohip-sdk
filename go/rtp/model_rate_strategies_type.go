/*
OPERA Cloud Rate API

APIs to cater for Rate Availability functionality in OPERA Cloud. <br /><br /> Rate Management provides all the tools you need to effectively define and manage the rate structures for a property in OPERA Cloud. Some of the things you can do include creating and managing rate codes, rate classes, rate categories, display sets, rate strategies, as well as managing promotion groups and codes.<br /><br /> Compatible with OPERA Cloud release 26.2.0.0.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2026 Oracle and/or its affiliates.</p>

API version: 26.2.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package rtp

import (
	"encoding/json"
)

// checks if the RateStrategiesType type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &RateStrategiesType{}

// RateStrategiesType The Rate strategy type.
type RateStrategiesType struct {
	// Rate Strategies.
	RateStrategy []RateStrategyType `json:"rateStrategy,omitempty"`
	// Hotel code.
	HotelId *string `json:"hotelId,omitempty"`
}

// NewRateStrategiesType instantiates a new RateStrategiesType object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRateStrategiesType() *RateStrategiesType {
	this := RateStrategiesType{}
	return &this
}

// NewRateStrategiesTypeWithDefaults instantiates a new RateStrategiesType object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRateStrategiesTypeWithDefaults() *RateStrategiesType {
	this := RateStrategiesType{}
	return &this
}

// GetRateStrategy returns the RateStrategy field value if set, zero value otherwise.
func (o *RateStrategiesType) GetRateStrategy() []RateStrategyType {
	if o == nil || IsNil(o.RateStrategy) {
		var ret []RateStrategyType
		return ret
	}
	return o.RateStrategy
}

// GetRateStrategyOk returns a tuple with the RateStrategy field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RateStrategiesType) GetRateStrategyOk() ([]RateStrategyType, bool) {
	if o == nil || IsNil(o.RateStrategy) {
		return nil, false
	}
	return o.RateStrategy, true
}

// HasRateStrategy returns a boolean if a field has been set.
func (o *RateStrategiesType) HasRateStrategy() bool {
	if o != nil && !IsNil(o.RateStrategy) {
		return true
	}

	return false
}

// SetRateStrategy gets a reference to the given []RateStrategyType and assigns it to the RateStrategy field.
func (o *RateStrategiesType) SetRateStrategy(v []RateStrategyType) {
	o.RateStrategy = v
}

// GetHotelId returns the HotelId field value if set, zero value otherwise.
func (o *RateStrategiesType) GetHotelId() string {
	if o == nil || IsNil(o.HotelId) {
		var ret string
		return ret
	}
	return *o.HotelId
}

// GetHotelIdOk returns a tuple with the HotelId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RateStrategiesType) GetHotelIdOk() (*string, bool) {
	if o == nil || IsNil(o.HotelId) {
		return nil, false
	}
	return o.HotelId, true
}

// HasHotelId returns a boolean if a field has been set.
func (o *RateStrategiesType) HasHotelId() bool {
	if o != nil && !IsNil(o.HotelId) {
		return true
	}

	return false
}

// SetHotelId gets a reference to the given string and assigns it to the HotelId field.
func (o *RateStrategiesType) SetHotelId(v string) {
	o.HotelId = &v
}

func (o RateStrategiesType) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o RateStrategiesType) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.RateStrategy) {
		toSerialize["rateStrategy"] = o.RateStrategy
	}
	if !IsNil(o.HotelId) {
		toSerialize["hotelId"] = o.HotelId
	}
	return toSerialize, nil
}

type NullableRateStrategiesType struct {
	value *RateStrategiesType
	isSet bool
}

func (v NullableRateStrategiesType) Get() *RateStrategiesType {
	return v.value
}

func (v *NullableRateStrategiesType) Set(val *RateStrategiesType) {
	v.value = val
	v.isSet = true
}

func (v NullableRateStrategiesType) IsSet() bool {
	return v.isSet
}

func (v *NullableRateStrategiesType) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRateStrategiesType(val *RateStrategiesType) *NullableRateStrategiesType {
	return &NullableRateStrategiesType{value: val, isSet: true}
}

func (v NullableRateStrategiesType) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRateStrategiesType) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



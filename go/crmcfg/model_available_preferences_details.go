/*
OPERA Cloud CRM Configuration API

APIs for Customer Relationship Management (profile) configuration, such as creating preferences, or address types.  It also includes Membership Configuration, where you can retrieve membership levels that are configured for a property, or create new membership enrollment codes .<br /><br /> Compatible with OPERA Cloud release 26.2.0.0.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2026 Oracle and/or its affiliates.</p>

API version: 26.2.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package crmcfg

import (
	"encoding/json"
)

// checks if the AvailablePreferencesDetails type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AvailablePreferencesDetails{}

// AvailablePreferencesDetails Response object for fetching available preferences for a property.
type AvailablePreferencesDetails struct {
	// A collection of preferences
	PreferenceCollection []PreferenceTypeType `json:"preferenceCollection,omitempty"`
	Links []InstanceLink `json:"links,omitempty"`
	// Used in conjunction with the Success element to define a business error.
	Warnings []WarningType `json:"warnings,omitempty"`
}

// NewAvailablePreferencesDetails instantiates a new AvailablePreferencesDetails object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAvailablePreferencesDetails() *AvailablePreferencesDetails {
	this := AvailablePreferencesDetails{}
	return &this
}

// NewAvailablePreferencesDetailsWithDefaults instantiates a new AvailablePreferencesDetails object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAvailablePreferencesDetailsWithDefaults() *AvailablePreferencesDetails {
	this := AvailablePreferencesDetails{}
	return &this
}

// GetPreferenceCollection returns the PreferenceCollection field value if set, zero value otherwise.
func (o *AvailablePreferencesDetails) GetPreferenceCollection() []PreferenceTypeType {
	if o == nil || IsNil(o.PreferenceCollection) {
		var ret []PreferenceTypeType
		return ret
	}
	return o.PreferenceCollection
}

// GetPreferenceCollectionOk returns a tuple with the PreferenceCollection field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AvailablePreferencesDetails) GetPreferenceCollectionOk() ([]PreferenceTypeType, bool) {
	if o == nil || IsNil(o.PreferenceCollection) {
		return nil, false
	}
	return o.PreferenceCollection, true
}

// HasPreferenceCollection returns a boolean if a field has been set.
func (o *AvailablePreferencesDetails) HasPreferenceCollection() bool {
	if o != nil && !IsNil(o.PreferenceCollection) {
		return true
	}

	return false
}

// SetPreferenceCollection gets a reference to the given []PreferenceTypeType and assigns it to the PreferenceCollection field.
func (o *AvailablePreferencesDetails) SetPreferenceCollection(v []PreferenceTypeType) {
	o.PreferenceCollection = v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *AvailablePreferencesDetails) GetLinks() []InstanceLink {
	if o == nil || IsNil(o.Links) {
		var ret []InstanceLink
		return ret
	}
	return o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AvailablePreferencesDetails) GetLinksOk() ([]InstanceLink, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *AvailablePreferencesDetails) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given []InstanceLink and assigns it to the Links field.
func (o *AvailablePreferencesDetails) SetLinks(v []InstanceLink) {
	o.Links = v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *AvailablePreferencesDetails) GetWarnings() []WarningType {
	if o == nil || IsNil(o.Warnings) {
		var ret []WarningType
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AvailablePreferencesDetails) GetWarningsOk() ([]WarningType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *AvailablePreferencesDetails) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []WarningType and assigns it to the Warnings field.
func (o *AvailablePreferencesDetails) SetWarnings(v []WarningType) {
	o.Warnings = v
}

func (o AvailablePreferencesDetails) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AvailablePreferencesDetails) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.PreferenceCollection) {
		toSerialize["preferenceCollection"] = o.PreferenceCollection
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableAvailablePreferencesDetails struct {
	value *AvailablePreferencesDetails
	isSet bool
}

func (v NullableAvailablePreferencesDetails) Get() *AvailablePreferencesDetails {
	return v.value
}

func (v *NullableAvailablePreferencesDetails) Set(val *AvailablePreferencesDetails) {
	v.value = val
	v.isSet = true
}

func (v NullableAvailablePreferencesDetails) IsSet() bool {
	return v.isSet
}

func (v *NullableAvailablePreferencesDetails) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAvailablePreferencesDetails(val *AvailablePreferencesDetails) *NullableAvailablePreferencesDetails {
	return &NullableAvailablePreferencesDetails{value: val, isSet: true}
}

func (v NullableAvailablePreferencesDetails) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAvailablePreferencesDetails) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



/*
OPERA Cloud Cashiering API

APIs to cater for Cashiering related functionality in OPERA Cloud.<br /><br /> Cashiering provides access to a guest folio, posting journals, receipt histories, currency calculations, credit card settlements, and check a guest out.<br /><br /> Compatible with OPERA Cloud release 26.2.0.0.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2026 Oracle and/or its affiliates.</p>

API version: 26.2.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package csh

import (
	"encoding/json"
)

// checks if the RoutingInstructionsDetails type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &RoutingInstructionsDetails{}

// RoutingInstructionsDetails Response object containing routing instructions for a reservation.
type RoutingInstructionsDetails struct {
	// A routing info object can either be of type Folio OR of type Room with its corresponding instructions.
	RoutingInstructions []RoutingInfoType `json:"routingInstructions,omitempty"`
	Links []InstanceLink `json:"links,omitempty"`
	// Used in conjunction with the Success element to define a business error.
	Warnings []WarningType `json:"warnings,omitempty"`
}

// NewRoutingInstructionsDetails instantiates a new RoutingInstructionsDetails object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRoutingInstructionsDetails() *RoutingInstructionsDetails {
	this := RoutingInstructionsDetails{}
	return &this
}

// NewRoutingInstructionsDetailsWithDefaults instantiates a new RoutingInstructionsDetails object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRoutingInstructionsDetailsWithDefaults() *RoutingInstructionsDetails {
	this := RoutingInstructionsDetails{}
	return &this
}

// GetRoutingInstructions returns the RoutingInstructions field value if set, zero value otherwise.
func (o *RoutingInstructionsDetails) GetRoutingInstructions() []RoutingInfoType {
	if o == nil || IsNil(o.RoutingInstructions) {
		var ret []RoutingInfoType
		return ret
	}
	return o.RoutingInstructions
}

// GetRoutingInstructionsOk returns a tuple with the RoutingInstructions field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RoutingInstructionsDetails) GetRoutingInstructionsOk() ([]RoutingInfoType, bool) {
	if o == nil || IsNil(o.RoutingInstructions) {
		return nil, false
	}
	return o.RoutingInstructions, true
}

// HasRoutingInstructions returns a boolean if a field has been set.
func (o *RoutingInstructionsDetails) HasRoutingInstructions() bool {
	if o != nil && !IsNil(o.RoutingInstructions) {
		return true
	}

	return false
}

// SetRoutingInstructions gets a reference to the given []RoutingInfoType and assigns it to the RoutingInstructions field.
func (o *RoutingInstructionsDetails) SetRoutingInstructions(v []RoutingInfoType) {
	o.RoutingInstructions = v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *RoutingInstructionsDetails) GetLinks() []InstanceLink {
	if o == nil || IsNil(o.Links) {
		var ret []InstanceLink
		return ret
	}
	return o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RoutingInstructionsDetails) GetLinksOk() ([]InstanceLink, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *RoutingInstructionsDetails) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given []InstanceLink and assigns it to the Links field.
func (o *RoutingInstructionsDetails) SetLinks(v []InstanceLink) {
	o.Links = v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *RoutingInstructionsDetails) GetWarnings() []WarningType {
	if o == nil || IsNil(o.Warnings) {
		var ret []WarningType
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RoutingInstructionsDetails) GetWarningsOk() ([]WarningType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *RoutingInstructionsDetails) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []WarningType and assigns it to the Warnings field.
func (o *RoutingInstructionsDetails) SetWarnings(v []WarningType) {
	o.Warnings = v
}

func (o RoutingInstructionsDetails) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o RoutingInstructionsDetails) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.RoutingInstructions) {
		toSerialize["routingInstructions"] = o.RoutingInstructions
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableRoutingInstructionsDetails struct {
	value *RoutingInstructionsDetails
	isSet bool
}

func (v NullableRoutingInstructionsDetails) Get() *RoutingInstructionsDetails {
	return v.value
}

func (v *NullableRoutingInstructionsDetails) Set(val *RoutingInstructionsDetails) {
	v.value = val
	v.isSet = true
}

func (v NullableRoutingInstructionsDetails) IsSet() bool {
	return v.isSet
}

func (v *NullableRoutingInstructionsDetails) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRoutingInstructionsDetails(val *RoutingInstructionsDetails) *NullableRoutingInstructionsDetails {
	return &NullableRoutingInstructionsDetails{value: val, isSet: true}
}

func (v NullableRoutingInstructionsDetails) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRoutingInstructionsDetails) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



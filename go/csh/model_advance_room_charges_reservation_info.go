/*
OPERA Cloud Cashiering API

APIs to cater for Cashiering related functionality in OPERA Cloud.<br /><br /> Cashiering provides access to a guest folio, posting journals, receipt histories, currency calculations, credit card settlements, and check a guest out.<br /><br /> Compatible with OPERA Cloud release 26.2.0.0.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2026 Oracle and/or its affiliates.</p>

API version: 26.2.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package csh

import (
	"encoding/json"
)

// checks if the AdvanceRoomChargesReservationInfo type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AdvanceRoomChargesReservationInfo{}

// AdvanceRoomChargesReservationInfo Response for the generate advance folio request. Post Advance Room Charges Request can return the below error codes : FOF00065 - Reservation ID or Resort was not supplied. FOF00147 - Reservation is currently not In-House. FOF00148 - Date range requested is not valid. FOF00149 - Advance Room Charges Posting was not completed successfully as there was an Error during posting. FOF00150 - Room Charges have already been posted for the requested dates. FOF00151 - Advance Room Charges could not be completed {the error message if any}.
type AdvanceRoomChargesReservationInfo struct {
	ReservationInfo *AdvanceRoomChargesInfoType `json:"reservationInfo,omitempty"`
	Links []InstanceLink `json:"links,omitempty"`
	// Used in conjunction with the Success element to define a business error.
	Warnings []WarningType `json:"warnings,omitempty"`
}

// NewAdvanceRoomChargesReservationInfo instantiates a new AdvanceRoomChargesReservationInfo object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAdvanceRoomChargesReservationInfo() *AdvanceRoomChargesReservationInfo {
	this := AdvanceRoomChargesReservationInfo{}
	return &this
}

// NewAdvanceRoomChargesReservationInfoWithDefaults instantiates a new AdvanceRoomChargesReservationInfo object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAdvanceRoomChargesReservationInfoWithDefaults() *AdvanceRoomChargesReservationInfo {
	this := AdvanceRoomChargesReservationInfo{}
	return &this
}

// GetReservationInfo returns the ReservationInfo field value if set, zero value otherwise.
func (o *AdvanceRoomChargesReservationInfo) GetReservationInfo() AdvanceRoomChargesInfoType {
	if o == nil || IsNil(o.ReservationInfo) {
		var ret AdvanceRoomChargesInfoType
		return ret
	}
	return *o.ReservationInfo
}

// GetReservationInfoOk returns a tuple with the ReservationInfo field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvanceRoomChargesReservationInfo) GetReservationInfoOk() (*AdvanceRoomChargesInfoType, bool) {
	if o == nil || IsNil(o.ReservationInfo) {
		return nil, false
	}
	return o.ReservationInfo, true
}

// HasReservationInfo returns a boolean if a field has been set.
func (o *AdvanceRoomChargesReservationInfo) HasReservationInfo() bool {
	if o != nil && !IsNil(o.ReservationInfo) {
		return true
	}

	return false
}

// SetReservationInfo gets a reference to the given AdvanceRoomChargesInfoType and assigns it to the ReservationInfo field.
func (o *AdvanceRoomChargesReservationInfo) SetReservationInfo(v AdvanceRoomChargesInfoType) {
	o.ReservationInfo = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *AdvanceRoomChargesReservationInfo) GetLinks() []InstanceLink {
	if o == nil || IsNil(o.Links) {
		var ret []InstanceLink
		return ret
	}
	return o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvanceRoomChargesReservationInfo) GetLinksOk() ([]InstanceLink, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *AdvanceRoomChargesReservationInfo) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given []InstanceLink and assigns it to the Links field.
func (o *AdvanceRoomChargesReservationInfo) SetLinks(v []InstanceLink) {
	o.Links = v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *AdvanceRoomChargesReservationInfo) GetWarnings() []WarningType {
	if o == nil || IsNil(o.Warnings) {
		var ret []WarningType
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdvanceRoomChargesReservationInfo) GetWarningsOk() ([]WarningType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *AdvanceRoomChargesReservationInfo) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []WarningType and assigns it to the Warnings field.
func (o *AdvanceRoomChargesReservationInfo) SetWarnings(v []WarningType) {
	o.Warnings = v
}

func (o AdvanceRoomChargesReservationInfo) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AdvanceRoomChargesReservationInfo) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.ReservationInfo) {
		toSerialize["reservationInfo"] = o.ReservationInfo
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableAdvanceRoomChargesReservationInfo struct {
	value *AdvanceRoomChargesReservationInfo
	isSet bool
}

func (v NullableAdvanceRoomChargesReservationInfo) Get() *AdvanceRoomChargesReservationInfo {
	return v.value
}

func (v *NullableAdvanceRoomChargesReservationInfo) Set(val *AdvanceRoomChargesReservationInfo) {
	v.value = val
	v.isSet = true
}

func (v NullableAdvanceRoomChargesReservationInfo) IsSet() bool {
	return v.isSet
}

func (v *NullableAdvanceRoomChargesReservationInfo) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAdvanceRoomChargesReservationInfo(val *AdvanceRoomChargesReservationInfo) *NullableAdvanceRoomChargesReservationInfo {
	return &NullableAdvanceRoomChargesReservationInfo{value: val, isSet: true}
}

func (v NullableAdvanceRoomChargesReservationInfo) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAdvanceRoomChargesReservationInfo) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



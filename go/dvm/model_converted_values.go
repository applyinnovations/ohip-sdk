/*
OPERA Cloud DataValueMapping Service API

APIs which offer external systems to config and use values different than what are configured in opera<br /><br /> Compatible with OPERA Cloud release 26.2.0.0.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2026 Oracle and/or its affiliates.</p>

API version: 26.2.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package dvm

import (
	"encoding/json"
)

// checks if the ConvertedValues type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ConvertedValues{}

// ConvertedValues struct for ConvertedValues
type ConvertedValues struct {
	// ExternalSystem for which the conversion is needed.
	ExternalSystem *string `json:"externalSystem,omitempty"`
	HotelInfo *ConversionHotelInfoType `json:"hotelInfo,omitempty"`
	// A collection of converted values.
	ConvertedValues []ConvertedValueType `json:"convertedValues,omitempty"`
	Links []InstanceLink `json:"links,omitempty"`
	// Used in conjunction with the Success element to define a business error.
	Warnings []WarningType `json:"warnings,omitempty"`
}

// NewConvertedValues instantiates a new ConvertedValues object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewConvertedValues() *ConvertedValues {
	this := ConvertedValues{}
	return &this
}

// NewConvertedValuesWithDefaults instantiates a new ConvertedValues object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewConvertedValuesWithDefaults() *ConvertedValues {
	this := ConvertedValues{}
	return &this
}

// GetExternalSystem returns the ExternalSystem field value if set, zero value otherwise.
func (o *ConvertedValues) GetExternalSystem() string {
	if o == nil || IsNil(o.ExternalSystem) {
		var ret string
		return ret
	}
	return *o.ExternalSystem
}

// GetExternalSystemOk returns a tuple with the ExternalSystem field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertedValues) GetExternalSystemOk() (*string, bool) {
	if o == nil || IsNil(o.ExternalSystem) {
		return nil, false
	}
	return o.ExternalSystem, true
}

// HasExternalSystem returns a boolean if a field has been set.
func (o *ConvertedValues) HasExternalSystem() bool {
	if o != nil && !IsNil(o.ExternalSystem) {
		return true
	}

	return false
}

// SetExternalSystem gets a reference to the given string and assigns it to the ExternalSystem field.
func (o *ConvertedValues) SetExternalSystem(v string) {
	o.ExternalSystem = &v
}

// GetHotelInfo returns the HotelInfo field value if set, zero value otherwise.
func (o *ConvertedValues) GetHotelInfo() ConversionHotelInfoType {
	if o == nil || IsNil(o.HotelInfo) {
		var ret ConversionHotelInfoType
		return ret
	}
	return *o.HotelInfo
}

// GetHotelInfoOk returns a tuple with the HotelInfo field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertedValues) GetHotelInfoOk() (*ConversionHotelInfoType, bool) {
	if o == nil || IsNil(o.HotelInfo) {
		return nil, false
	}
	return o.HotelInfo, true
}

// HasHotelInfo returns a boolean if a field has been set.
func (o *ConvertedValues) HasHotelInfo() bool {
	if o != nil && !IsNil(o.HotelInfo) {
		return true
	}

	return false
}

// SetHotelInfo gets a reference to the given ConversionHotelInfoType and assigns it to the HotelInfo field.
func (o *ConvertedValues) SetHotelInfo(v ConversionHotelInfoType) {
	o.HotelInfo = &v
}

// GetConvertedValues returns the ConvertedValues field value if set, zero value otherwise.
func (o *ConvertedValues) GetConvertedValues() []ConvertedValueType {
	if o == nil || IsNil(o.ConvertedValues) {
		var ret []ConvertedValueType
		return ret
	}
	return o.ConvertedValues
}

// GetConvertedValuesOk returns a tuple with the ConvertedValues field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertedValues) GetConvertedValuesOk() ([]ConvertedValueType, bool) {
	if o == nil || IsNil(o.ConvertedValues) {
		return nil, false
	}
	return o.ConvertedValues, true
}

// HasConvertedValues returns a boolean if a field has been set.
func (o *ConvertedValues) HasConvertedValues() bool {
	if o != nil && !IsNil(o.ConvertedValues) {
		return true
	}

	return false
}

// SetConvertedValues gets a reference to the given []ConvertedValueType and assigns it to the ConvertedValues field.
func (o *ConvertedValues) SetConvertedValues(v []ConvertedValueType) {
	o.ConvertedValues = v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *ConvertedValues) GetLinks() []InstanceLink {
	if o == nil || IsNil(o.Links) {
		var ret []InstanceLink
		return ret
	}
	return o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertedValues) GetLinksOk() ([]InstanceLink, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *ConvertedValues) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given []InstanceLink and assigns it to the Links field.
func (o *ConvertedValues) SetLinks(v []InstanceLink) {
	o.Links = v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *ConvertedValues) GetWarnings() []WarningType {
	if o == nil || IsNil(o.Warnings) {
		var ret []WarningType
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvertedValues) GetWarningsOk() ([]WarningType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *ConvertedValues) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []WarningType and assigns it to the Warnings field.
func (o *ConvertedValues) SetWarnings(v []WarningType) {
	o.Warnings = v
}

func (o ConvertedValues) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ConvertedValues) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.ExternalSystem) {
		toSerialize["externalSystem"] = o.ExternalSystem
	}
	if !IsNil(o.HotelInfo) {
		toSerialize["hotelInfo"] = o.HotelInfo
	}
	if !IsNil(o.ConvertedValues) {
		toSerialize["convertedValues"] = o.ConvertedValues
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableConvertedValues struct {
	value *ConvertedValues
	isSet bool
}

func (v NullableConvertedValues) Get() *ConvertedValues {
	return v.value
}

func (v *NullableConvertedValues) Set(val *ConvertedValues) {
	v.value = val
	v.isSet = true
}

func (v NullableConvertedValues) IsSet() bool {
	return v.isSet
}

func (v *NullableConvertedValues) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableConvertedValues(val *ConvertedValues) *NullableConvertedValues {
	return &NullableConvertedValues{value: val, isSet: true}
}

func (v NullableConvertedValues) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableConvertedValues) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



/*
OPERA Cloud Event Configuration API

This API caters for Event Configuration in OPERA Cloud. <br /><There are operations to post, update, fetch and delete codes such as item inventory, function spaces, menu items and many more.<br /><br /> Compatible with OPERA Cloud release 26.2.0.0.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2026 Oracle and/or its affiliates.</p>

API version: 26.2.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package evmcfg

import (
	"encoding/json"
)

// checks if the FunctionSpaceSetupStylesDetails type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &FunctionSpaceSetupStylesDetails{}

// FunctionSpaceSetupStylesDetails Response object for fetching function space setup styles.
type FunctionSpaceSetupStylesDetails struct {
	// List of the function space setup styles to be configured or fetched
	SetupStyles []FunctionSpaceSetupStyleType `json:"setupStyles,omitempty"`
	Links []InstanceLink `json:"links,omitempty"`
	// Used in conjunction with the Success element to define a business error.
	Warnings []WarningType `json:"warnings,omitempty"`
}

// NewFunctionSpaceSetupStylesDetails instantiates a new FunctionSpaceSetupStylesDetails object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFunctionSpaceSetupStylesDetails() *FunctionSpaceSetupStylesDetails {
	this := FunctionSpaceSetupStylesDetails{}
	return &this
}

// NewFunctionSpaceSetupStylesDetailsWithDefaults instantiates a new FunctionSpaceSetupStylesDetails object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFunctionSpaceSetupStylesDetailsWithDefaults() *FunctionSpaceSetupStylesDetails {
	this := FunctionSpaceSetupStylesDetails{}
	return &this
}

// GetSetupStyles returns the SetupStyles field value if set, zero value otherwise.
func (o *FunctionSpaceSetupStylesDetails) GetSetupStyles() []FunctionSpaceSetupStyleType {
	if o == nil || IsNil(o.SetupStyles) {
		var ret []FunctionSpaceSetupStyleType
		return ret
	}
	return o.SetupStyles
}

// GetSetupStylesOk returns a tuple with the SetupStyles field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FunctionSpaceSetupStylesDetails) GetSetupStylesOk() ([]FunctionSpaceSetupStyleType, bool) {
	if o == nil || IsNil(o.SetupStyles) {
		return nil, false
	}
	return o.SetupStyles, true
}

// HasSetupStyles returns a boolean if a field has been set.
func (o *FunctionSpaceSetupStylesDetails) HasSetupStyles() bool {
	if o != nil && !IsNil(o.SetupStyles) {
		return true
	}

	return false
}

// SetSetupStyles gets a reference to the given []FunctionSpaceSetupStyleType and assigns it to the SetupStyles field.
func (o *FunctionSpaceSetupStylesDetails) SetSetupStyles(v []FunctionSpaceSetupStyleType) {
	o.SetupStyles = v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *FunctionSpaceSetupStylesDetails) GetLinks() []InstanceLink {
	if o == nil || IsNil(o.Links) {
		var ret []InstanceLink
		return ret
	}
	return o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FunctionSpaceSetupStylesDetails) GetLinksOk() ([]InstanceLink, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *FunctionSpaceSetupStylesDetails) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given []InstanceLink and assigns it to the Links field.
func (o *FunctionSpaceSetupStylesDetails) SetLinks(v []InstanceLink) {
	o.Links = v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *FunctionSpaceSetupStylesDetails) GetWarnings() []WarningType {
	if o == nil || IsNil(o.Warnings) {
		var ret []WarningType
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FunctionSpaceSetupStylesDetails) GetWarningsOk() ([]WarningType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *FunctionSpaceSetupStylesDetails) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []WarningType and assigns it to the Warnings field.
func (o *FunctionSpaceSetupStylesDetails) SetWarnings(v []WarningType) {
	o.Warnings = v
}

func (o FunctionSpaceSetupStylesDetails) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o FunctionSpaceSetupStylesDetails) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.SetupStyles) {
		toSerialize["setupStyles"] = o.SetupStyles
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableFunctionSpaceSetupStylesDetails struct {
	value *FunctionSpaceSetupStylesDetails
	isSet bool
}

func (v NullableFunctionSpaceSetupStylesDetails) Get() *FunctionSpaceSetupStylesDetails {
	return v.value
}

func (v *NullableFunctionSpaceSetupStylesDetails) Set(val *FunctionSpaceSetupStylesDetails) {
	v.value = val
	v.isSet = true
}

func (v NullableFunctionSpaceSetupStylesDetails) IsSet() bool {
	return v.isSet
}

func (v *NullableFunctionSpaceSetupStylesDetails) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFunctionSpaceSetupStylesDetails(val *FunctionSpaceSetupStylesDetails) *NullableFunctionSpaceSetupStylesDetails {
	return &NullableFunctionSpaceSetupStylesDetails{value: val, isSet: true}
}

func (v NullableFunctionSpaceSetupStylesDetails) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFunctionSpaceSetupStylesDetails) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



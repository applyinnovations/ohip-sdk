...
```golang
import tokenexchange "github.com/applyinnovations/ohip-sdk/go/tokenexchange/v3"
```
...
For using other server than the one defined on index 0 set context value `tokenexchange.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `tokenexchange.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `tokenexchange.ContextOperationServerIndices` and `tokenexchange.ContextOperationServerVariables` context maps.
...
auth := context.WithValue(
		context.Background(),
		tokenexchange.ContextAPIKeys,
		map[string]tokenexchange.APIKey{

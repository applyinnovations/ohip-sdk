/*
OPERA Cloud Block API

APIs to cater for Business Block functionality in OPERA Cloud. <br /><br /> A block is a group of rooms held for guests who are attending an event, meeting, or function. You can create blocks for family reunions, business conferences, weddings, and so on. You can also set aside rooms for the event (block).<br /><br /> Compatible with OPERA Cloud release 22.3.0.0.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2022 Oracle and/or its affiliates.</p>

API version: 22.3.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the PostBlockPostingMasterRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PostBlockPostingMasterRequest{}

// PostBlockPostingMasterRequest struct for PostBlockPostingMasterRequest
type PostBlockPostingMasterRequest struct {
	Criteria *CreateBlockPostingMasterCriteria `json:"criteria,omitempty"`
	Links []InstanceLink `json:"links,omitempty"`
	// Used in conjunction with the Success element to define a business error.
	Warnings []WarningType `json:"warnings,omitempty"`
}

// NewPostBlockPostingMasterRequest instantiates a new PostBlockPostingMasterRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPostBlockPostingMasterRequest() *PostBlockPostingMasterRequest {
	this := PostBlockPostingMasterRequest{}
	return &this
}

// NewPostBlockPostingMasterRequestWithDefaults instantiates a new PostBlockPostingMasterRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPostBlockPostingMasterRequestWithDefaults() *PostBlockPostingMasterRequest {
	this := PostBlockPostingMasterRequest{}
	return &this
}

// GetCriteria returns the Criteria field value if set, zero value otherwise.
func (o *PostBlockPostingMasterRequest) GetCriteria() CreateBlockPostingMasterCriteria {
	if o == nil || IsNil(o.Criteria) {
		var ret CreateBlockPostingMasterCriteria
		return ret
	}
	return *o.Criteria
}

// GetCriteriaOk returns a tuple with the Criteria field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostBlockPostingMasterRequest) GetCriteriaOk() (*CreateBlockPostingMasterCriteria, bool) {
	if o == nil || IsNil(o.Criteria) {
		return nil, false
	}
	return o.Criteria, true
}

// HasCriteria returns a boolean if a field has been set.
func (o *PostBlockPostingMasterRequest) HasCriteria() bool {
	if o != nil && !IsNil(o.Criteria) {
		return true
	}

	return false
}

// SetCriteria gets a reference to the given CreateBlockPostingMasterCriteria and assigns it to the Criteria field.
func (o *PostBlockPostingMasterRequest) SetCriteria(v CreateBlockPostingMasterCriteria) {
	o.Criteria = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PostBlockPostingMasterRequest) GetLinks() []InstanceLink {
	if o == nil || IsNil(o.Links) {
		var ret []InstanceLink
		return ret
	}
	return o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostBlockPostingMasterRequest) GetLinksOk() ([]InstanceLink, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PostBlockPostingMasterRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given []InstanceLink and assigns it to the Links field.
func (o *PostBlockPostingMasterRequest) SetLinks(v []InstanceLink) {
	o.Links = v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PostBlockPostingMasterRequest) GetWarnings() []WarningType {
	if o == nil || IsNil(o.Warnings) {
		var ret []WarningType
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostBlockPostingMasterRequest) GetWarningsOk() ([]WarningType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PostBlockPostingMasterRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []WarningType and assigns it to the Warnings field.
func (o *PostBlockPostingMasterRequest) SetWarnings(v []WarningType) {
	o.Warnings = v
}

func (o PostBlockPostingMasterRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PostBlockPostingMasterRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Criteria) {
		toSerialize["criteria"] = o.Criteria
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePostBlockPostingMasterRequest struct {
	value *PostBlockPostingMasterRequest
	isSet bool
}

func (v NullablePostBlockPostingMasterRequest) Get() *PostBlockPostingMasterRequest {
	return v.value
}

func (v *NullablePostBlockPostingMasterRequest) Set(val *PostBlockPostingMasterRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePostBlockPostingMasterRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePostBlockPostingMasterRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePostBlockPostingMasterRequest(val *PostBlockPostingMasterRequest) *NullablePostBlockPostingMasterRequest {
	return &NullablePostBlockPostingMasterRequest{value: val, isSet: true}
}

func (v NullablePostBlockPostingMasterRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePostBlockPostingMasterRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



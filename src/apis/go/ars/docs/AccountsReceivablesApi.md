...
> OperaVersion PingAccountsReceivableService(ctx).Authorization(authorization).XAppKey(xAppKey).XHotelid(xHotelid).XExternalsystem(xExternalsystem).AcceptLanguage(acceptLanguage).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    apiClient := openapiclient.NewAPIClient(configuration)
    resp, r, err := apiClient.AccountsReceivablesApi.PingAccountsReceivableService(context.Background()).Authorization(authorization).XAppKey(xAppKey).XHotelid(xHotelid).XExternalsystem(xExternalsystem).AcceptLanguage(acceptLanguage).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `AccountsReceivablesApi.PingAccountsReceivableService``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `PingAccountsReceivableService`: OperaVersion
    fmt.Fprintf(os.Stdout, "Response from `AccountsReceivablesApi.PingAccountsReceivableService`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiPingAccountsReceivableServiceRequest struct via the builder pattern

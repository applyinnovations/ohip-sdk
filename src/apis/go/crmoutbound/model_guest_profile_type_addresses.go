/*
OPERA Cloud Customer Relationship Management Outbound API

APIs to cater for Customer Relationship Management external (outbound) functionality with OPERA. These APIs facilitate various operations related to getting data from an external system, and inserting it into OPERA.<br /><br /> Compatible with OPERA Cloud release 22.3.0.0.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2022 Oracle and/or its affiliates.</p>

API version: 22.3.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the GuestProfileTypeAddresses type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GuestProfileTypeAddresses{}

// GuestProfileTypeAddresses List of customer addresses.
type GuestProfileTypeAddresses struct {
	// Collection of Detailed information on an address for the customer.
	AddressInfo []AddressInfoType `json:"addressInfo,omitempty"`
	// Indicates whether all the records are included in the response or not. Absence of the attribute values should be consider as all rows fetched in the response.
	AllRowsFetched *bool `json:"allRowsFetched,omitempty"`
	// Total number of rows queried
	TotalRows *int32 `json:"totalRows,omitempty"`
}

// NewGuestProfileTypeAddresses instantiates a new GuestProfileTypeAddresses object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGuestProfileTypeAddresses() *GuestProfileTypeAddresses {
	this := GuestProfileTypeAddresses{}
	return &this
}

// NewGuestProfileTypeAddressesWithDefaults instantiates a new GuestProfileTypeAddresses object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGuestProfileTypeAddressesWithDefaults() *GuestProfileTypeAddresses {
	this := GuestProfileTypeAddresses{}
	return &this
}

// GetAddressInfo returns the AddressInfo field value if set, zero value otherwise.
func (o *GuestProfileTypeAddresses) GetAddressInfo() []AddressInfoType {
	if o == nil || IsNil(o.AddressInfo) {
		var ret []AddressInfoType
		return ret
	}
	return o.AddressInfo
}

// GetAddressInfoOk returns a tuple with the AddressInfo field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GuestProfileTypeAddresses) GetAddressInfoOk() ([]AddressInfoType, bool) {
	if o == nil || IsNil(o.AddressInfo) {
		return nil, false
	}
	return o.AddressInfo, true
}

// HasAddressInfo returns a boolean if a field has been set.
func (o *GuestProfileTypeAddresses) HasAddressInfo() bool {
	if o != nil && !IsNil(o.AddressInfo) {
		return true
	}

	return false
}

// SetAddressInfo gets a reference to the given []AddressInfoType and assigns it to the AddressInfo field.
func (o *GuestProfileTypeAddresses) SetAddressInfo(v []AddressInfoType) {
	o.AddressInfo = v
}

// GetAllRowsFetched returns the AllRowsFetched field value if set, zero value otherwise.
func (o *GuestProfileTypeAddresses) GetAllRowsFetched() bool {
	if o == nil || IsNil(o.AllRowsFetched) {
		var ret bool
		return ret
	}
	return *o.AllRowsFetched
}

// GetAllRowsFetchedOk returns a tuple with the AllRowsFetched field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GuestProfileTypeAddresses) GetAllRowsFetchedOk() (*bool, bool) {
	if o == nil || IsNil(o.AllRowsFetched) {
		return nil, false
	}
	return o.AllRowsFetched, true
}

// HasAllRowsFetched returns a boolean if a field has been set.
func (o *GuestProfileTypeAddresses) HasAllRowsFetched() bool {
	if o != nil && !IsNil(o.AllRowsFetched) {
		return true
	}

	return false
}

// SetAllRowsFetched gets a reference to the given bool and assigns it to the AllRowsFetched field.
func (o *GuestProfileTypeAddresses) SetAllRowsFetched(v bool) {
	o.AllRowsFetched = &v
}

// GetTotalRows returns the TotalRows field value if set, zero value otherwise.
func (o *GuestProfileTypeAddresses) GetTotalRows() int32 {
	if o == nil || IsNil(o.TotalRows) {
		var ret int32
		return ret
	}
	return *o.TotalRows
}

// GetTotalRowsOk returns a tuple with the TotalRows field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GuestProfileTypeAddresses) GetTotalRowsOk() (*int32, bool) {
	if o == nil || IsNil(o.TotalRows) {
		return nil, false
	}
	return o.TotalRows, true
}

// HasTotalRows returns a boolean if a field has been set.
func (o *GuestProfileTypeAddresses) HasTotalRows() bool {
	if o != nil && !IsNil(o.TotalRows) {
		return true
	}

	return false
}

// SetTotalRows gets a reference to the given int32 and assigns it to the TotalRows field.
func (o *GuestProfileTypeAddresses) SetTotalRows(v int32) {
	o.TotalRows = &v
}

func (o GuestProfileTypeAddresses) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GuestProfileTypeAddresses) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.AddressInfo) {
		toSerialize["addressInfo"] = o.AddressInfo
	}
	if !IsNil(o.AllRowsFetched) {
		toSerialize["allRowsFetched"] = o.AllRowsFetched
	}
	if !IsNil(o.TotalRows) {
		toSerialize["totalRows"] = o.TotalRows
	}
	return toSerialize, nil
}

type NullableGuestProfileTypeAddresses struct {
	value *GuestProfileTypeAddresses
	isSet bool
}

func (v NullableGuestProfileTypeAddresses) Get() *GuestProfileTypeAddresses {
	return v.value
}

func (v *NullableGuestProfileTypeAddresses) Set(val *GuestProfileTypeAddresses) {
	v.value = val
	v.isSet = true
}

func (v NullableGuestProfileTypeAddresses) IsSet() bool {
	return v.isSet
}

func (v *NullableGuestProfileTypeAddresses) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGuestProfileTypeAddresses(val *GuestProfileTypeAddresses) *NullableGuestProfileTypeAddresses {
	return &NullableGuestProfileTypeAddresses{value: val, isSet: true}
}

func (v NullableGuestProfileTypeAddresses) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGuestProfileTypeAddresses) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



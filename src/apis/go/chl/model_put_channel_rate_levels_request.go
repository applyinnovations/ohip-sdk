/*
OPERA Cloud Channel Configuration API

APIs to cater for Channel Management functionality in OPERA Cloud. <br /><br /> Channel Management allows a property to configure and administer channels such as OTAs, and web channels, covering functionality such as channel configuration, availability, inventory and restrictions.<br /><br /> Compatible with OPERA Cloud release 22.3.0.0.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2022 Oracle and/or its affiliates.</p>

API version: 22.3.0.0
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the PutChannelRateLevelsRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PutChannelRateLevelsRequest{}

// PutChannelRateLevelsRequest struct for PutChannelRateLevelsRequest
type PutChannelRateLevelsRequest struct {
	RateLevels *ChannelRateLevelsRateLevels `json:"rateLevels,omitempty"`
	Links []InstanceLink `json:"links,omitempty"`
	// Used in conjunction with the Success element to define a business error.
	Warnings []WarningType `json:"warnings,omitempty"`
}

// NewPutChannelRateLevelsRequest instantiates a new PutChannelRateLevelsRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPutChannelRateLevelsRequest() *PutChannelRateLevelsRequest {
	this := PutChannelRateLevelsRequest{}
	return &this
}

// NewPutChannelRateLevelsRequestWithDefaults instantiates a new PutChannelRateLevelsRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPutChannelRateLevelsRequestWithDefaults() *PutChannelRateLevelsRequest {
	this := PutChannelRateLevelsRequest{}
	return &this
}

// GetRateLevels returns the RateLevels field value if set, zero value otherwise.
func (o *PutChannelRateLevelsRequest) GetRateLevels() ChannelRateLevelsRateLevels {
	if o == nil || IsNil(o.RateLevels) {
		var ret ChannelRateLevelsRateLevels
		return ret
	}
	return *o.RateLevels
}

// GetRateLevelsOk returns a tuple with the RateLevels field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutChannelRateLevelsRequest) GetRateLevelsOk() (*ChannelRateLevelsRateLevels, bool) {
	if o == nil || IsNil(o.RateLevels) {
		return nil, false
	}
	return o.RateLevels, true
}

// HasRateLevels returns a boolean if a field has been set.
func (o *PutChannelRateLevelsRequest) HasRateLevels() bool {
	if o != nil && !IsNil(o.RateLevels) {
		return true
	}

	return false
}

// SetRateLevels gets a reference to the given ChannelRateLevelsRateLevels and assigns it to the RateLevels field.
func (o *PutChannelRateLevelsRequest) SetRateLevels(v ChannelRateLevelsRateLevels) {
	o.RateLevels = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PutChannelRateLevelsRequest) GetLinks() []InstanceLink {
	if o == nil || IsNil(o.Links) {
		var ret []InstanceLink
		return ret
	}
	return o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutChannelRateLevelsRequest) GetLinksOk() ([]InstanceLink, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PutChannelRateLevelsRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given []InstanceLink and assigns it to the Links field.
func (o *PutChannelRateLevelsRequest) SetLinks(v []InstanceLink) {
	o.Links = v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PutChannelRateLevelsRequest) GetWarnings() []WarningType {
	if o == nil || IsNil(o.Warnings) {
		var ret []WarningType
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutChannelRateLevelsRequest) GetWarningsOk() ([]WarningType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PutChannelRateLevelsRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []WarningType and assigns it to the Warnings field.
func (o *PutChannelRateLevelsRequest) SetWarnings(v []WarningType) {
	o.Warnings = v
}

func (o PutChannelRateLevelsRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PutChannelRateLevelsRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.RateLevels) {
		toSerialize["rateLevels"] = o.RateLevels
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePutChannelRateLevelsRequest struct {
	value *PutChannelRateLevelsRequest
	isSet bool
}

func (v NullablePutChannelRateLevelsRequest) Get() *PutChannelRateLevelsRequest {
	return v.value
}

func (v *NullablePutChannelRateLevelsRequest) Set(val *PutChannelRateLevelsRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePutChannelRateLevelsRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePutChannelRateLevelsRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePutChannelRateLevelsRequest(val *PutChannelRateLevelsRequest) *NullablePutChannelRateLevelsRequest {
	return &NullablePutChannelRateLevelsRequest{value: val, isSet: true}
}

func (v NullablePutChannelRateLevelsRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePutChannelRateLevelsRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



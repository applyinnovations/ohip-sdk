/* tslint:disable */
/* eslint-disable */
/**
 * OPERA Cloud Room Configuration API
 * APIs to cater for room configuration, such as configuring room types, room Classes, creating new room features, or updating housekeeping room maintenance reasons.<br /><br /> Compatible with OPERA Cloud release 24.4.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2024 Oracle and/or its affiliates.</p>
 *
 * The version of the OpenAPI document: 24.4
 * Contact: hospitality_apis_ww_grp@oracle.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import type { InstanceLink } from './InstanceLink';
import {
    InstanceLinkFromJSON,
    InstanceLinkFromJSONTyped,
    InstanceLinkToJSON,
} from './InstanceLink';
import type { TemplateRoomFeatureConfigType } from './TemplateRoomFeatureConfigType';
import {
    TemplateRoomFeatureConfigTypeFromJSON,
    TemplateRoomFeatureConfigTypeFromJSONTyped,
    TemplateRoomFeatureConfigTypeToJSON,
} from './TemplateRoomFeatureConfigType';
import type { WarningType } from './WarningType';
import {
    WarningTypeFromJSON,
    WarningTypeFromJSONTyped,
    WarningTypeToJSON,
} from './WarningType';

/**
 * Response object to fetch template room features.
 * @export
 * @interface TemplateRoomFeaturesDetails
 */
export interface TemplateRoomFeaturesDetails {
    /**
     * Template room feature details.
     * @type {Array<TemplateRoomFeatureConfigType>}
     * @memberof TemplateRoomFeaturesDetails
     */
    templateRoomFeatures?: Array<TemplateRoomFeatureConfigType>;
    /**
     * 
     * @type {Array<InstanceLink>}
     * @memberof TemplateRoomFeaturesDetails
     */
    links?: Array<InstanceLink>;
    /**
     * Used in conjunction with the Success element to define a business error.
     * @type {Array<WarningType>}
     * @memberof TemplateRoomFeaturesDetails
     */
    warnings?: Array<WarningType>;
}

/**
 * Check if a given object implements the TemplateRoomFeaturesDetails interface.
 */
export function instanceOfTemplateRoomFeaturesDetails(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function TemplateRoomFeaturesDetailsFromJSON(json: any): TemplateRoomFeaturesDetails {
    return TemplateRoomFeaturesDetailsFromJSONTyped(json, false);
}

export function TemplateRoomFeaturesDetailsFromJSONTyped(json: any, ignoreDiscriminator: boolean): TemplateRoomFeaturesDetails {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'templateRoomFeatures': !exists(json, 'templateRoomFeatures') ? undefined : ((json['templateRoomFeatures'] as Array<any>).map(TemplateRoomFeatureConfigTypeFromJSON)),
        'links': !exists(json, 'links') ? undefined : ((json['links'] as Array<any>).map(InstanceLinkFromJSON)),
        'warnings': !exists(json, 'warnings') ? undefined : ((json['warnings'] as Array<any>).map(WarningTypeFromJSON)),
    };
}

export function TemplateRoomFeaturesDetailsToJSON(value?: TemplateRoomFeaturesDetails | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'templateRoomFeatures': value.templateRoomFeatures === undefined ? undefined : ((value.templateRoomFeatures as Array<any>).map(TemplateRoomFeatureConfigTypeToJSON)),
        'links': value.links === undefined ? undefined : ((value.links as Array<any>).map(InstanceLinkToJSON)),
        'warnings': value.warnings === undefined ? undefined : ((value.warnings as Array<any>).map(WarningTypeToJSON)),
    };
}


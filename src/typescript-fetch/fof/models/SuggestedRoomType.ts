/* tslint:disable */
/* eslint-disable */
/**
 * OPERA Cloud Front Desk Operations Service
 * APIs to cater for Front Desk Operations and Front Desk Statistic functionality in OPERA Cloud. <br /><br /> Front Desk features some of the most commonly used operations in OPERA Cloud, such as managing guest arrivals, managing in-house guests, and managing guest departures. Some additional tasks you can complete from the Front Desk menu are room searches, room assignments, and quick check outs as well as opening folios, creating registration cards, setting wake up calls, and sending messages to guests.<br /><br /> Compatible with OPERA Cloud release 24.4.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2024 Oracle and/or its affiliates.</p>
 *
 * The version of the OpenAPI document: 24.4
 * Contact: hospitality_apis_ww_grp@oracle.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
/**
 * List of suggested rooms and component rooms.
 * @export
 * @interface SuggestedRoomType
 */
export interface SuggestedRoomType {
    /**
     * List of component Room number belongs to a room.
     * @type {Array<string>}
     * @memberof SuggestedRoomType
     */
    componentRoomNumber?: Array<string>;
    /**
     * Code of the room.
     * @type {string}
     * @memberof SuggestedRoomType
     */
    roomId?: string;
}

/**
 * Check if a given object implements the SuggestedRoomType interface.
 */
export function instanceOfSuggestedRoomType(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function SuggestedRoomTypeFromJSON(json: any): SuggestedRoomType {
    return SuggestedRoomTypeFromJSONTyped(json, false);
}

export function SuggestedRoomTypeFromJSONTyped(json: any, ignoreDiscriminator: boolean): SuggestedRoomType {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'componentRoomNumber': !exists(json, 'componentRoomNumber') ? undefined : json['componentRoomNumber'],
        'roomId': !exists(json, 'roomId') ? undefined : json['roomId'],
    };
}

export function SuggestedRoomTypeToJSON(value?: SuggestedRoomType | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'componentRoomNumber': value.componentRoomNumber,
        'roomId': value.roomId,
    };
}


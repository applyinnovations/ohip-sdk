/*
OPERA Cloud Reservation Master Data Management API

APIs to cater for Reservation Configuration in OPERA Cloud. In this module you can retrieve, create, modify or delete configuration related to Reservations, Blocks and Leisure Management.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package rsvcfg

import (
	"encoding/json"
)

// checks if the PutReservationColorsRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PutReservationColorsRequest{}

// PutReservationColorsRequest struct for PutReservationColorsRequest
type PutReservationColorsRequest struct {
	Links *Links `json:"links,omitempty"`
	ReservationColors *ReservationColorsType `json:"reservationColors,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewPutReservationColorsRequest instantiates a new PutReservationColorsRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPutReservationColorsRequest() *PutReservationColorsRequest {
	this := PutReservationColorsRequest{}
	return &this
}

// NewPutReservationColorsRequestWithDefaults instantiates a new PutReservationColorsRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPutReservationColorsRequestWithDefaults() *PutReservationColorsRequest {
	this := PutReservationColorsRequest{}
	return &this
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PutReservationColorsRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutReservationColorsRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PutReservationColorsRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *PutReservationColorsRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetReservationColors returns the ReservationColors field value if set, zero value otherwise.
func (o *PutReservationColorsRequest) GetReservationColors() ReservationColorsType {
	if o == nil || IsNil(o.ReservationColors) {
		var ret ReservationColorsType
		return ret
	}
	return *o.ReservationColors
}

// GetReservationColorsOk returns a tuple with the ReservationColors field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutReservationColorsRequest) GetReservationColorsOk() (*ReservationColorsType, bool) {
	if o == nil || IsNil(o.ReservationColors) {
		return nil, false
	}
	return o.ReservationColors, true
}

// HasReservationColors returns a boolean if a field has been set.
func (o *PutReservationColorsRequest) HasReservationColors() bool {
	if o != nil && !IsNil(o.ReservationColors) {
		return true
	}

	return false
}

// SetReservationColors gets a reference to the given ReservationColorsType and assigns it to the ReservationColors field.
func (o *PutReservationColorsRequest) SetReservationColors(v ReservationColorsType) {
	o.ReservationColors = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PutReservationColorsRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutReservationColorsRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PutReservationColorsRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *PutReservationColorsRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o PutReservationColorsRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PutReservationColorsRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.ReservationColors) {
		toSerialize["reservationColors"] = o.ReservationColors
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePutReservationColorsRequest struct {
	value *PutReservationColorsRequest
	isSet bool
}

func (v NullablePutReservationColorsRequest) Get() *PutReservationColorsRequest {
	return v.value
}

func (v *NullablePutReservationColorsRequest) Set(val *PutReservationColorsRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePutReservationColorsRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePutReservationColorsRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePutReservationColorsRequest(val *PutReservationColorsRequest) *NullablePutReservationColorsRequest {
	return &NullablePutReservationColorsRequest{value: val, isSet: true}
}

func (v NullablePutReservationColorsRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePutReservationColorsRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



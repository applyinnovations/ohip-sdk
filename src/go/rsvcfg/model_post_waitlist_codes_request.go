/*
OPERA Cloud Reservation Master Data Management API

APIs to cater for Reservation Configuration in OPERA Cloud. In this module you can retrieve, create, modify or delete configuration related to Reservations, Blocks and Leisure Management.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package rsvcfg

import (
	"encoding/json"
)

// checks if the PostWaitlistCodesRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PostWaitlistCodesRequest{}

// PostWaitlistCodesRequest struct for PostWaitlistCodesRequest
type PostWaitlistCodesRequest struct {
	Links *Links `json:"links,omitempty"`
	WaitlistCodes *WaitlistCodesType `json:"waitlistCodes,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewPostWaitlistCodesRequest instantiates a new PostWaitlistCodesRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPostWaitlistCodesRequest() *PostWaitlistCodesRequest {
	this := PostWaitlistCodesRequest{}
	return &this
}

// NewPostWaitlistCodesRequestWithDefaults instantiates a new PostWaitlistCodesRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPostWaitlistCodesRequestWithDefaults() *PostWaitlistCodesRequest {
	this := PostWaitlistCodesRequest{}
	return &this
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PostWaitlistCodesRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostWaitlistCodesRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PostWaitlistCodesRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *PostWaitlistCodesRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetWaitlistCodes returns the WaitlistCodes field value if set, zero value otherwise.
func (o *PostWaitlistCodesRequest) GetWaitlistCodes() WaitlistCodesType {
	if o == nil || IsNil(o.WaitlistCodes) {
		var ret WaitlistCodesType
		return ret
	}
	return *o.WaitlistCodes
}

// GetWaitlistCodesOk returns a tuple with the WaitlistCodes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostWaitlistCodesRequest) GetWaitlistCodesOk() (*WaitlistCodesType, bool) {
	if o == nil || IsNil(o.WaitlistCodes) {
		return nil, false
	}
	return o.WaitlistCodes, true
}

// HasWaitlistCodes returns a boolean if a field has been set.
func (o *PostWaitlistCodesRequest) HasWaitlistCodes() bool {
	if o != nil && !IsNil(o.WaitlistCodes) {
		return true
	}

	return false
}

// SetWaitlistCodes gets a reference to the given WaitlistCodesType and assigns it to the WaitlistCodes field.
func (o *PostWaitlistCodesRequest) SetWaitlistCodes(v WaitlistCodesType) {
	o.WaitlistCodes = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PostWaitlistCodesRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostWaitlistCodesRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PostWaitlistCodesRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *PostWaitlistCodesRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o PostWaitlistCodesRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PostWaitlistCodesRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.WaitlistCodes) {
		toSerialize["waitlistCodes"] = o.WaitlistCodes
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePostWaitlistCodesRequest struct {
	value *PostWaitlistCodesRequest
	isSet bool
}

func (v NullablePostWaitlistCodesRequest) Get() *PostWaitlistCodesRequest {
	return v.value
}

func (v *NullablePostWaitlistCodesRequest) Set(val *PostWaitlistCodesRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePostWaitlistCodesRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePostWaitlistCodesRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePostWaitlistCodesRequest(val *PostWaitlistCodesRequest) *NullablePostWaitlistCodesRequest {
	return &NullablePostWaitlistCodesRequest{value: val, isSet: true}
}

func (v NullablePostWaitlistCodesRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePostWaitlistCodesRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



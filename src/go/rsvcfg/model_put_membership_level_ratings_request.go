/*
OPERA Cloud Reservation Master Data Management API

APIs to cater for Reservation Configuration in OPERA Cloud. In this module you can retrieve, create, modify or delete configuration related to Reservations, Blocks and Leisure Management.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package rsvcfg

import (
	"encoding/json"
)

// checks if the PutMembershipLevelRatingsRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PutMembershipLevelRatingsRequest{}

// PutMembershipLevelRatingsRequest struct for PutMembershipLevelRatingsRequest
type PutMembershipLevelRatingsRequest struct {
	Links *Links `json:"links,omitempty"`
	MembershipLevelRatingsInfo *MembershipLevelRatingsInfoType `json:"membershipLevelRatingsInfo,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewPutMembershipLevelRatingsRequest instantiates a new PutMembershipLevelRatingsRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPutMembershipLevelRatingsRequest() *PutMembershipLevelRatingsRequest {
	this := PutMembershipLevelRatingsRequest{}
	return &this
}

// NewPutMembershipLevelRatingsRequestWithDefaults instantiates a new PutMembershipLevelRatingsRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPutMembershipLevelRatingsRequestWithDefaults() *PutMembershipLevelRatingsRequest {
	this := PutMembershipLevelRatingsRequest{}
	return &this
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PutMembershipLevelRatingsRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutMembershipLevelRatingsRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PutMembershipLevelRatingsRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *PutMembershipLevelRatingsRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetMembershipLevelRatingsInfo returns the MembershipLevelRatingsInfo field value if set, zero value otherwise.
func (o *PutMembershipLevelRatingsRequest) GetMembershipLevelRatingsInfo() MembershipLevelRatingsInfoType {
	if o == nil || IsNil(o.MembershipLevelRatingsInfo) {
		var ret MembershipLevelRatingsInfoType
		return ret
	}
	return *o.MembershipLevelRatingsInfo
}

// GetMembershipLevelRatingsInfoOk returns a tuple with the MembershipLevelRatingsInfo field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutMembershipLevelRatingsRequest) GetMembershipLevelRatingsInfoOk() (*MembershipLevelRatingsInfoType, bool) {
	if o == nil || IsNil(o.MembershipLevelRatingsInfo) {
		return nil, false
	}
	return o.MembershipLevelRatingsInfo, true
}

// HasMembershipLevelRatingsInfo returns a boolean if a field has been set.
func (o *PutMembershipLevelRatingsRequest) HasMembershipLevelRatingsInfo() bool {
	if o != nil && !IsNil(o.MembershipLevelRatingsInfo) {
		return true
	}

	return false
}

// SetMembershipLevelRatingsInfo gets a reference to the given MembershipLevelRatingsInfoType and assigns it to the MembershipLevelRatingsInfo field.
func (o *PutMembershipLevelRatingsRequest) SetMembershipLevelRatingsInfo(v MembershipLevelRatingsInfoType) {
	o.MembershipLevelRatingsInfo = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PutMembershipLevelRatingsRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutMembershipLevelRatingsRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PutMembershipLevelRatingsRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *PutMembershipLevelRatingsRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o PutMembershipLevelRatingsRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PutMembershipLevelRatingsRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.MembershipLevelRatingsInfo) {
		toSerialize["membershipLevelRatingsInfo"] = o.MembershipLevelRatingsInfo
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePutMembershipLevelRatingsRequest struct {
	value *PutMembershipLevelRatingsRequest
	isSet bool
}

func (v NullablePutMembershipLevelRatingsRequest) Get() *PutMembershipLevelRatingsRequest {
	return v.value
}

func (v *NullablePutMembershipLevelRatingsRequest) Set(val *PutMembershipLevelRatingsRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePutMembershipLevelRatingsRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePutMembershipLevelRatingsRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePutMembershipLevelRatingsRequest(val *PutMembershipLevelRatingsRequest) *NullablePutMembershipLevelRatingsRequest {
	return &NullablePutMembershipLevelRatingsRequest{value: val, isSet: true}
}

func (v NullablePutMembershipLevelRatingsRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePutMembershipLevelRatingsRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



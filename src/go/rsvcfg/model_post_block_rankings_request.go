/*
OPERA Cloud Reservation Master Data Management API

APIs to cater for Reservation Configuration in OPERA Cloud. In this module you can retrieve, create, modify or delete configuration related to Reservations, Blocks and Leisure Management.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package rsvcfg

import (
	"encoding/json"
)

// checks if the PostBlockRankingsRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PostBlockRankingsRequest{}

// PostBlockRankingsRequest struct for PostBlockRankingsRequest
type PostBlockRankingsRequest struct {
	BlockRankings *BlockRankingsType `json:"blockRankings,omitempty"`
	Links *Links `json:"links,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewPostBlockRankingsRequest instantiates a new PostBlockRankingsRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPostBlockRankingsRequest() *PostBlockRankingsRequest {
	this := PostBlockRankingsRequest{}
	return &this
}

// NewPostBlockRankingsRequestWithDefaults instantiates a new PostBlockRankingsRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPostBlockRankingsRequestWithDefaults() *PostBlockRankingsRequest {
	this := PostBlockRankingsRequest{}
	return &this
}

// GetBlockRankings returns the BlockRankings field value if set, zero value otherwise.
func (o *PostBlockRankingsRequest) GetBlockRankings() BlockRankingsType {
	if o == nil || IsNil(o.BlockRankings) {
		var ret BlockRankingsType
		return ret
	}
	return *o.BlockRankings
}

// GetBlockRankingsOk returns a tuple with the BlockRankings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostBlockRankingsRequest) GetBlockRankingsOk() (*BlockRankingsType, bool) {
	if o == nil || IsNil(o.BlockRankings) {
		return nil, false
	}
	return o.BlockRankings, true
}

// HasBlockRankings returns a boolean if a field has been set.
func (o *PostBlockRankingsRequest) HasBlockRankings() bool {
	if o != nil && !IsNil(o.BlockRankings) {
		return true
	}

	return false
}

// SetBlockRankings gets a reference to the given BlockRankingsType and assigns it to the BlockRankings field.
func (o *PostBlockRankingsRequest) SetBlockRankings(v BlockRankingsType) {
	o.BlockRankings = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PostBlockRankingsRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostBlockRankingsRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PostBlockRankingsRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *PostBlockRankingsRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PostBlockRankingsRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostBlockRankingsRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PostBlockRankingsRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *PostBlockRankingsRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o PostBlockRankingsRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PostBlockRankingsRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.BlockRankings) {
		toSerialize["blockRankings"] = o.BlockRankings
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePostBlockRankingsRequest struct {
	value *PostBlockRankingsRequest
	isSet bool
}

func (v NullablePostBlockRankingsRequest) Get() *PostBlockRankingsRequest {
	return v.value
}

func (v *NullablePostBlockRankingsRequest) Set(val *PostBlockRankingsRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePostBlockRankingsRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePostBlockRankingsRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePostBlockRankingsRequest(val *PostBlockRankingsRequest) *NullablePostBlockRankingsRequest {
	return &NullablePostBlockRankingsRequest{value: val, isSet: true}
}

func (v NullablePostBlockRankingsRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePostBlockRankingsRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



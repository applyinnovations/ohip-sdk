/*
OPERA Cloud Reservation Master Data Management API

APIs to cater for Reservation Configuration in OPERA Cloud. In this module you can retrieve, create, modify or delete configuration related to Reservations, Blocks and Leisure Management.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package rsvcfg

import (
	"encoding/json"
)

// checks if the PostEntryPointsRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PostEntryPointsRequest{}

// PostEntryPointsRequest struct for PostEntryPointsRequest
type PostEntryPointsRequest struct {
	EntryPoints *EntryPointsType `json:"entryPoints,omitempty"`
	Links *Links `json:"links,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewPostEntryPointsRequest instantiates a new PostEntryPointsRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPostEntryPointsRequest() *PostEntryPointsRequest {
	this := PostEntryPointsRequest{}
	return &this
}

// NewPostEntryPointsRequestWithDefaults instantiates a new PostEntryPointsRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPostEntryPointsRequestWithDefaults() *PostEntryPointsRequest {
	this := PostEntryPointsRequest{}
	return &this
}

// GetEntryPoints returns the EntryPoints field value if set, zero value otherwise.
func (o *PostEntryPointsRequest) GetEntryPoints() EntryPointsType {
	if o == nil || IsNil(o.EntryPoints) {
		var ret EntryPointsType
		return ret
	}
	return *o.EntryPoints
}

// GetEntryPointsOk returns a tuple with the EntryPoints field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostEntryPointsRequest) GetEntryPointsOk() (*EntryPointsType, bool) {
	if o == nil || IsNil(o.EntryPoints) {
		return nil, false
	}
	return o.EntryPoints, true
}

// HasEntryPoints returns a boolean if a field has been set.
func (o *PostEntryPointsRequest) HasEntryPoints() bool {
	if o != nil && !IsNil(o.EntryPoints) {
		return true
	}

	return false
}

// SetEntryPoints gets a reference to the given EntryPointsType and assigns it to the EntryPoints field.
func (o *PostEntryPointsRequest) SetEntryPoints(v EntryPointsType) {
	o.EntryPoints = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PostEntryPointsRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostEntryPointsRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PostEntryPointsRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *PostEntryPointsRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PostEntryPointsRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostEntryPointsRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PostEntryPointsRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *PostEntryPointsRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o PostEntryPointsRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PostEntryPointsRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.EntryPoints) {
		toSerialize["entryPoints"] = o.EntryPoints
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePostEntryPointsRequest struct {
	value *PostEntryPointsRequest
	isSet bool
}

func (v NullablePostEntryPointsRequest) Get() *PostEntryPointsRequest {
	return v.value
}

func (v *NullablePostEntryPointsRequest) Set(val *PostEntryPointsRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePostEntryPointsRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePostEntryPointsRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePostEntryPointsRequest(val *PostEntryPointsRequest) *NullablePostEntryPointsRequest {
	return &NullablePostEntryPointsRequest{value: val, isSet: true}
}

func (v NullablePostEntryPointsRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePostEntryPointsRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



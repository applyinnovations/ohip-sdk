/*
OPERA Cloud Front Desk Configuration API

APIs to cater for Front Desk Configuration in OPERA Cloud. Here you can find operations to get, post, put and delete front desk codes such as commission codes, transaction groups, codes & subgroups, articles, payment methods and credit card types.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fofcfg

import (
	"encoding/json"
)

// checks if the ChangeCustomTaxTypesRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ChangeCustomTaxTypesRequest{}

// ChangeCustomTaxTypesRequest struct for ChangeCustomTaxTypesRequest
type ChangeCustomTaxTypesRequest struct {
	CustomTaxTypes *CustomTaxTypesType `json:"customTaxTypes,omitempty"`
	Links *Links `json:"links,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewChangeCustomTaxTypesRequest instantiates a new ChangeCustomTaxTypesRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewChangeCustomTaxTypesRequest() *ChangeCustomTaxTypesRequest {
	this := ChangeCustomTaxTypesRequest{}
	return &this
}

// NewChangeCustomTaxTypesRequestWithDefaults instantiates a new ChangeCustomTaxTypesRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewChangeCustomTaxTypesRequestWithDefaults() *ChangeCustomTaxTypesRequest {
	this := ChangeCustomTaxTypesRequest{}
	return &this
}

// GetCustomTaxTypes returns the CustomTaxTypes field value if set, zero value otherwise.
func (o *ChangeCustomTaxTypesRequest) GetCustomTaxTypes() CustomTaxTypesType {
	if o == nil || IsNil(o.CustomTaxTypes) {
		var ret CustomTaxTypesType
		return ret
	}
	return *o.CustomTaxTypes
}

// GetCustomTaxTypesOk returns a tuple with the CustomTaxTypes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeCustomTaxTypesRequest) GetCustomTaxTypesOk() (*CustomTaxTypesType, bool) {
	if o == nil || IsNil(o.CustomTaxTypes) {
		return nil, false
	}
	return o.CustomTaxTypes, true
}

// HasCustomTaxTypes returns a boolean if a field has been set.
func (o *ChangeCustomTaxTypesRequest) HasCustomTaxTypes() bool {
	if o != nil && !IsNil(o.CustomTaxTypes) {
		return true
	}

	return false
}

// SetCustomTaxTypes gets a reference to the given CustomTaxTypesType and assigns it to the CustomTaxTypes field.
func (o *ChangeCustomTaxTypesRequest) SetCustomTaxTypes(v CustomTaxTypesType) {
	o.CustomTaxTypes = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *ChangeCustomTaxTypesRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeCustomTaxTypesRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *ChangeCustomTaxTypesRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *ChangeCustomTaxTypesRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *ChangeCustomTaxTypesRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeCustomTaxTypesRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *ChangeCustomTaxTypesRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *ChangeCustomTaxTypesRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o ChangeCustomTaxTypesRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ChangeCustomTaxTypesRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.CustomTaxTypes) {
		toSerialize["customTaxTypes"] = o.CustomTaxTypes
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableChangeCustomTaxTypesRequest struct {
	value *ChangeCustomTaxTypesRequest
	isSet bool
}

func (v NullableChangeCustomTaxTypesRequest) Get() *ChangeCustomTaxTypesRequest {
	return v.value
}

func (v *NullableChangeCustomTaxTypesRequest) Set(val *ChangeCustomTaxTypesRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableChangeCustomTaxTypesRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableChangeCustomTaxTypesRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableChangeCustomTaxTypesRequest(val *ChangeCustomTaxTypesRequest) *NullableChangeCustomTaxTypesRequest {
	return &NullableChangeCustomTaxTypesRequest{value: val, isSet: true}
}

func (v NullableChangeCustomTaxTypesRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableChangeCustomTaxTypesRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



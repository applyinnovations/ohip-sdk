/*
OPERA Cloud Front Desk Configuration API

APIs to cater for Front Desk Configuration in OPERA Cloud. Here you can find operations to get, post, put and delete front desk codes such as commission codes, transaction groups, codes & subgroups, articles, payment methods and credit card types.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fofcfg

import (
	"encoding/json"
)

// checks if the PostAutoFolioSettlementTypesRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PostAutoFolioSettlementTypesRequest{}

// PostAutoFolioSettlementTypesRequest struct for PostAutoFolioSettlementTypesRequest
type PostAutoFolioSettlementTypesRequest struct {
	AutoFolioSettlementTypes *AutoFolioSettlementTypesType `json:"autoFolioSettlementTypes,omitempty"`
	Links *Links `json:"links,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewPostAutoFolioSettlementTypesRequest instantiates a new PostAutoFolioSettlementTypesRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPostAutoFolioSettlementTypesRequest() *PostAutoFolioSettlementTypesRequest {
	this := PostAutoFolioSettlementTypesRequest{}
	return &this
}

// NewPostAutoFolioSettlementTypesRequestWithDefaults instantiates a new PostAutoFolioSettlementTypesRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPostAutoFolioSettlementTypesRequestWithDefaults() *PostAutoFolioSettlementTypesRequest {
	this := PostAutoFolioSettlementTypesRequest{}
	return &this
}

// GetAutoFolioSettlementTypes returns the AutoFolioSettlementTypes field value if set, zero value otherwise.
func (o *PostAutoFolioSettlementTypesRequest) GetAutoFolioSettlementTypes() AutoFolioSettlementTypesType {
	if o == nil || IsNil(o.AutoFolioSettlementTypes) {
		var ret AutoFolioSettlementTypesType
		return ret
	}
	return *o.AutoFolioSettlementTypes
}

// GetAutoFolioSettlementTypesOk returns a tuple with the AutoFolioSettlementTypes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostAutoFolioSettlementTypesRequest) GetAutoFolioSettlementTypesOk() (*AutoFolioSettlementTypesType, bool) {
	if o == nil || IsNil(o.AutoFolioSettlementTypes) {
		return nil, false
	}
	return o.AutoFolioSettlementTypes, true
}

// HasAutoFolioSettlementTypes returns a boolean if a field has been set.
func (o *PostAutoFolioSettlementTypesRequest) HasAutoFolioSettlementTypes() bool {
	if o != nil && !IsNil(o.AutoFolioSettlementTypes) {
		return true
	}

	return false
}

// SetAutoFolioSettlementTypes gets a reference to the given AutoFolioSettlementTypesType and assigns it to the AutoFolioSettlementTypes field.
func (o *PostAutoFolioSettlementTypesRequest) SetAutoFolioSettlementTypes(v AutoFolioSettlementTypesType) {
	o.AutoFolioSettlementTypes = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PostAutoFolioSettlementTypesRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostAutoFolioSettlementTypesRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PostAutoFolioSettlementTypesRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *PostAutoFolioSettlementTypesRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PostAutoFolioSettlementTypesRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostAutoFolioSettlementTypesRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PostAutoFolioSettlementTypesRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *PostAutoFolioSettlementTypesRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o PostAutoFolioSettlementTypesRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PostAutoFolioSettlementTypesRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.AutoFolioSettlementTypes) {
		toSerialize["autoFolioSettlementTypes"] = o.AutoFolioSettlementTypes
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePostAutoFolioSettlementTypesRequest struct {
	value *PostAutoFolioSettlementTypesRequest
	isSet bool
}

func (v NullablePostAutoFolioSettlementTypesRequest) Get() *PostAutoFolioSettlementTypesRequest {
	return v.value
}

func (v *NullablePostAutoFolioSettlementTypesRequest) Set(val *PostAutoFolioSettlementTypesRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePostAutoFolioSettlementTypesRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePostAutoFolioSettlementTypesRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePostAutoFolioSettlementTypesRequest(val *PostAutoFolioSettlementTypesRequest) *NullablePostAutoFolioSettlementTypesRequest {
	return &NullablePostAutoFolioSettlementTypesRequest{value: val, isSet: true}
}

func (v NullablePostAutoFolioSettlementTypesRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePostAutoFolioSettlementTypesRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



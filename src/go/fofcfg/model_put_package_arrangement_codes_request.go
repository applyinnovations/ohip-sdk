/*
OPERA Cloud Front Desk Configuration API

APIs to cater for Front Desk Configuration in OPERA Cloud. Here you can find operations to get, post, put and delete front desk codes such as commission codes, transaction groups, codes & subgroups, articles, payment methods and credit card types.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fofcfg

import (
	"encoding/json"
)

// checks if the PutPackageArrangementCodesRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PutPackageArrangementCodesRequest{}

// PutPackageArrangementCodesRequest struct for PutPackageArrangementCodesRequest
type PutPackageArrangementCodesRequest struct {
	Links *Links `json:"links,omitempty"`
	PackageArrangementCodes *PackageArrangementCodesType `json:"packageArrangementCodes,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewPutPackageArrangementCodesRequest instantiates a new PutPackageArrangementCodesRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPutPackageArrangementCodesRequest() *PutPackageArrangementCodesRequest {
	this := PutPackageArrangementCodesRequest{}
	return &this
}

// NewPutPackageArrangementCodesRequestWithDefaults instantiates a new PutPackageArrangementCodesRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPutPackageArrangementCodesRequestWithDefaults() *PutPackageArrangementCodesRequest {
	this := PutPackageArrangementCodesRequest{}
	return &this
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PutPackageArrangementCodesRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutPackageArrangementCodesRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PutPackageArrangementCodesRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *PutPackageArrangementCodesRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetPackageArrangementCodes returns the PackageArrangementCodes field value if set, zero value otherwise.
func (o *PutPackageArrangementCodesRequest) GetPackageArrangementCodes() PackageArrangementCodesType {
	if o == nil || IsNil(o.PackageArrangementCodes) {
		var ret PackageArrangementCodesType
		return ret
	}
	return *o.PackageArrangementCodes
}

// GetPackageArrangementCodesOk returns a tuple with the PackageArrangementCodes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutPackageArrangementCodesRequest) GetPackageArrangementCodesOk() (*PackageArrangementCodesType, bool) {
	if o == nil || IsNil(o.PackageArrangementCodes) {
		return nil, false
	}
	return o.PackageArrangementCodes, true
}

// HasPackageArrangementCodes returns a boolean if a field has been set.
func (o *PutPackageArrangementCodesRequest) HasPackageArrangementCodes() bool {
	if o != nil && !IsNil(o.PackageArrangementCodes) {
		return true
	}

	return false
}

// SetPackageArrangementCodes gets a reference to the given PackageArrangementCodesType and assigns it to the PackageArrangementCodes field.
func (o *PutPackageArrangementCodesRequest) SetPackageArrangementCodes(v PackageArrangementCodesType) {
	o.PackageArrangementCodes = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PutPackageArrangementCodesRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutPackageArrangementCodesRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PutPackageArrangementCodesRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *PutPackageArrangementCodesRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o PutPackageArrangementCodesRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PutPackageArrangementCodesRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.PackageArrangementCodes) {
		toSerialize["packageArrangementCodes"] = o.PackageArrangementCodes
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePutPackageArrangementCodesRequest struct {
	value *PutPackageArrangementCodesRequest
	isSet bool
}

func (v NullablePutPackageArrangementCodesRequest) Get() *PutPackageArrangementCodesRequest {
	return v.value
}

func (v *NullablePutPackageArrangementCodesRequest) Set(val *PutPackageArrangementCodesRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePutPackageArrangementCodesRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePutPackageArrangementCodesRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePutPackageArrangementCodesRequest(val *PutPackageArrangementCodesRequest) *NullablePutPackageArrangementCodesRequest {
	return &NullablePutPackageArrangementCodesRequest{value: val, isSet: true}
}

func (v NullablePutPackageArrangementCodesRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePutPackageArrangementCodesRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



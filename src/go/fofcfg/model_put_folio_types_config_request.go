/*
OPERA Cloud Front Desk Configuration API

APIs to cater for Front Desk Configuration in OPERA Cloud. Here you can find operations to get, post, put and delete front desk codes such as commission codes, transaction groups, codes & subgroups, articles, payment methods and credit card types.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fofcfg

import (
	"encoding/json"
)

// checks if the PutFolioTypesConfigRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PutFolioTypesConfigRequest{}

// PutFolioTypesConfigRequest struct for PutFolioTypesConfigRequest
type PutFolioTypesConfigRequest struct {
	FolioTypes *FolioTypesConfigType `json:"folioTypes,omitempty"`
	Links *Links `json:"links,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewPutFolioTypesConfigRequest instantiates a new PutFolioTypesConfigRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPutFolioTypesConfigRequest() *PutFolioTypesConfigRequest {
	this := PutFolioTypesConfigRequest{}
	return &this
}

// NewPutFolioTypesConfigRequestWithDefaults instantiates a new PutFolioTypesConfigRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPutFolioTypesConfigRequestWithDefaults() *PutFolioTypesConfigRequest {
	this := PutFolioTypesConfigRequest{}
	return &this
}

// GetFolioTypes returns the FolioTypes field value if set, zero value otherwise.
func (o *PutFolioTypesConfigRequest) GetFolioTypes() FolioTypesConfigType {
	if o == nil || IsNil(o.FolioTypes) {
		var ret FolioTypesConfigType
		return ret
	}
	return *o.FolioTypes
}

// GetFolioTypesOk returns a tuple with the FolioTypes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutFolioTypesConfigRequest) GetFolioTypesOk() (*FolioTypesConfigType, bool) {
	if o == nil || IsNil(o.FolioTypes) {
		return nil, false
	}
	return o.FolioTypes, true
}

// HasFolioTypes returns a boolean if a field has been set.
func (o *PutFolioTypesConfigRequest) HasFolioTypes() bool {
	if o != nil && !IsNil(o.FolioTypes) {
		return true
	}

	return false
}

// SetFolioTypes gets a reference to the given FolioTypesConfigType and assigns it to the FolioTypes field.
func (o *PutFolioTypesConfigRequest) SetFolioTypes(v FolioTypesConfigType) {
	o.FolioTypes = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PutFolioTypesConfigRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutFolioTypesConfigRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PutFolioTypesConfigRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *PutFolioTypesConfigRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PutFolioTypesConfigRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutFolioTypesConfigRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PutFolioTypesConfigRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *PutFolioTypesConfigRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o PutFolioTypesConfigRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PutFolioTypesConfigRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.FolioTypes) {
		toSerialize["folioTypes"] = o.FolioTypes
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePutFolioTypesConfigRequest struct {
	value *PutFolioTypesConfigRequest
	isSet bool
}

func (v NullablePutFolioTypesConfigRequest) Get() *PutFolioTypesConfigRequest {
	return v.value
}

func (v *NullablePutFolioTypesConfigRequest) Set(val *PutFolioTypesConfigRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePutFolioTypesConfigRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePutFolioTypesConfigRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePutFolioTypesConfigRequest(val *PutFolioTypesConfigRequest) *NullablePutFolioTypesConfigRequest {
	return &NullablePutFolioTypesConfigRequest{value: val, isSet: true}
}

func (v NullablePutFolioTypesConfigRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePutFolioTypesConfigRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



/*
OPERA Cloud Front Desk Configuration API

APIs to cater for Front Desk Configuration in OPERA Cloud. Here you can find operations to get, post, put and delete front desk codes such as commission codes, transaction groups, codes & subgroups, articles, payment methods and credit card types.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fofcfg

import (
	"encoding/json"
)

// checks if the PutConfigBankAccountsRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PutConfigBankAccountsRequest{}

// PutConfigBankAccountsRequest struct for PutConfigBankAccountsRequest
type PutConfigBankAccountsRequest struct {
	BankAccounts *ConfigBankAccountsType `json:"bankAccounts,omitempty"`
	Links *Links `json:"links,omitempty"`
	// Update default confirmation response
	UpdateDefault *bool `json:"updateDefault,omitempty"`
	// Update default for currency confirmation response
	UpdateDefaultForCurrency *bool `json:"updateDefaultForCurrency,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewPutConfigBankAccountsRequest instantiates a new PutConfigBankAccountsRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPutConfigBankAccountsRequest() *PutConfigBankAccountsRequest {
	this := PutConfigBankAccountsRequest{}
	return &this
}

// NewPutConfigBankAccountsRequestWithDefaults instantiates a new PutConfigBankAccountsRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPutConfigBankAccountsRequestWithDefaults() *PutConfigBankAccountsRequest {
	this := PutConfigBankAccountsRequest{}
	return &this
}

// GetBankAccounts returns the BankAccounts field value if set, zero value otherwise.
func (o *PutConfigBankAccountsRequest) GetBankAccounts() ConfigBankAccountsType {
	if o == nil || IsNil(o.BankAccounts) {
		var ret ConfigBankAccountsType
		return ret
	}
	return *o.BankAccounts
}

// GetBankAccountsOk returns a tuple with the BankAccounts field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutConfigBankAccountsRequest) GetBankAccountsOk() (*ConfigBankAccountsType, bool) {
	if o == nil || IsNil(o.BankAccounts) {
		return nil, false
	}
	return o.BankAccounts, true
}

// HasBankAccounts returns a boolean if a field has been set.
func (o *PutConfigBankAccountsRequest) HasBankAccounts() bool {
	if o != nil && !IsNil(o.BankAccounts) {
		return true
	}

	return false
}

// SetBankAccounts gets a reference to the given ConfigBankAccountsType and assigns it to the BankAccounts field.
func (o *PutConfigBankAccountsRequest) SetBankAccounts(v ConfigBankAccountsType) {
	o.BankAccounts = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PutConfigBankAccountsRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutConfigBankAccountsRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PutConfigBankAccountsRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *PutConfigBankAccountsRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetUpdateDefault returns the UpdateDefault field value if set, zero value otherwise.
func (o *PutConfigBankAccountsRequest) GetUpdateDefault() bool {
	if o == nil || IsNil(o.UpdateDefault) {
		var ret bool
		return ret
	}
	return *o.UpdateDefault
}

// GetUpdateDefaultOk returns a tuple with the UpdateDefault field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutConfigBankAccountsRequest) GetUpdateDefaultOk() (*bool, bool) {
	if o == nil || IsNil(o.UpdateDefault) {
		return nil, false
	}
	return o.UpdateDefault, true
}

// HasUpdateDefault returns a boolean if a field has been set.
func (o *PutConfigBankAccountsRequest) HasUpdateDefault() bool {
	if o != nil && !IsNil(o.UpdateDefault) {
		return true
	}

	return false
}

// SetUpdateDefault gets a reference to the given bool and assigns it to the UpdateDefault field.
func (o *PutConfigBankAccountsRequest) SetUpdateDefault(v bool) {
	o.UpdateDefault = &v
}

// GetUpdateDefaultForCurrency returns the UpdateDefaultForCurrency field value if set, zero value otherwise.
func (o *PutConfigBankAccountsRequest) GetUpdateDefaultForCurrency() bool {
	if o == nil || IsNil(o.UpdateDefaultForCurrency) {
		var ret bool
		return ret
	}
	return *o.UpdateDefaultForCurrency
}

// GetUpdateDefaultForCurrencyOk returns a tuple with the UpdateDefaultForCurrency field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutConfigBankAccountsRequest) GetUpdateDefaultForCurrencyOk() (*bool, bool) {
	if o == nil || IsNil(o.UpdateDefaultForCurrency) {
		return nil, false
	}
	return o.UpdateDefaultForCurrency, true
}

// HasUpdateDefaultForCurrency returns a boolean if a field has been set.
func (o *PutConfigBankAccountsRequest) HasUpdateDefaultForCurrency() bool {
	if o != nil && !IsNil(o.UpdateDefaultForCurrency) {
		return true
	}

	return false
}

// SetUpdateDefaultForCurrency gets a reference to the given bool and assigns it to the UpdateDefaultForCurrency field.
func (o *PutConfigBankAccountsRequest) SetUpdateDefaultForCurrency(v bool) {
	o.UpdateDefaultForCurrency = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PutConfigBankAccountsRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutConfigBankAccountsRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PutConfigBankAccountsRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *PutConfigBankAccountsRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o PutConfigBankAccountsRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PutConfigBankAccountsRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.BankAccounts) {
		toSerialize["bankAccounts"] = o.BankAccounts
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.UpdateDefault) {
		toSerialize["updateDefault"] = o.UpdateDefault
	}
	if !IsNil(o.UpdateDefaultForCurrency) {
		toSerialize["updateDefaultForCurrency"] = o.UpdateDefaultForCurrency
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePutConfigBankAccountsRequest struct {
	value *PutConfigBankAccountsRequest
	isSet bool
}

func (v NullablePutConfigBankAccountsRequest) Get() *PutConfigBankAccountsRequest {
	return v.value
}

func (v *NullablePutConfigBankAccountsRequest) Set(val *PutConfigBankAccountsRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePutConfigBankAccountsRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePutConfigBankAccountsRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePutConfigBankAccountsRequest(val *PutConfigBankAccountsRequest) *NullablePutConfigBankAccountsRequest {
	return &NullablePutConfigBankAccountsRequest{value: val, isSet: true}
}

func (v NullablePutConfigBankAccountsRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePutConfigBankAccountsRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



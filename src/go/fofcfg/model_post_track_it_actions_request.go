/*
OPERA Cloud Front Desk Configuration API

APIs to cater for Front Desk Configuration in OPERA Cloud. Here you can find operations to get, post, put and delete front desk codes such as commission codes, transaction groups, codes & subgroups, articles, payment methods and credit card types.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fofcfg

import (
	"encoding/json"
)

// checks if the PostTrackItActionsRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PostTrackItActionsRequest{}

// PostTrackItActionsRequest struct for PostTrackItActionsRequest
type PostTrackItActionsRequest struct {
	Links *Links `json:"links,omitempty"`
	TrackItActions *TrackItActionsConfigType `json:"trackItActions,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewPostTrackItActionsRequest instantiates a new PostTrackItActionsRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPostTrackItActionsRequest() *PostTrackItActionsRequest {
	this := PostTrackItActionsRequest{}
	return &this
}

// NewPostTrackItActionsRequestWithDefaults instantiates a new PostTrackItActionsRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPostTrackItActionsRequestWithDefaults() *PostTrackItActionsRequest {
	this := PostTrackItActionsRequest{}
	return &this
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PostTrackItActionsRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostTrackItActionsRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PostTrackItActionsRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *PostTrackItActionsRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetTrackItActions returns the TrackItActions field value if set, zero value otherwise.
func (o *PostTrackItActionsRequest) GetTrackItActions() TrackItActionsConfigType {
	if o == nil || IsNil(o.TrackItActions) {
		var ret TrackItActionsConfigType
		return ret
	}
	return *o.TrackItActions
}

// GetTrackItActionsOk returns a tuple with the TrackItActions field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostTrackItActionsRequest) GetTrackItActionsOk() (*TrackItActionsConfigType, bool) {
	if o == nil || IsNil(o.TrackItActions) {
		return nil, false
	}
	return o.TrackItActions, true
}

// HasTrackItActions returns a boolean if a field has been set.
func (o *PostTrackItActionsRequest) HasTrackItActions() bool {
	if o != nil && !IsNil(o.TrackItActions) {
		return true
	}

	return false
}

// SetTrackItActions gets a reference to the given TrackItActionsConfigType and assigns it to the TrackItActions field.
func (o *PostTrackItActionsRequest) SetTrackItActions(v TrackItActionsConfigType) {
	o.TrackItActions = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PostTrackItActionsRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostTrackItActionsRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PostTrackItActionsRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *PostTrackItActionsRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o PostTrackItActionsRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PostTrackItActionsRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.TrackItActions) {
		toSerialize["trackItActions"] = o.TrackItActions
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePostTrackItActionsRequest struct {
	value *PostTrackItActionsRequest
	isSet bool
}

func (v NullablePostTrackItActionsRequest) Get() *PostTrackItActionsRequest {
	return v.value
}

func (v *NullablePostTrackItActionsRequest) Set(val *PostTrackItActionsRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePostTrackItActionsRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePostTrackItActionsRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePostTrackItActionsRequest(val *PostTrackItActionsRequest) *NullablePostTrackItActionsRequest {
	return &NullablePostTrackItActionsRequest{value: val, isSet: true}
}

func (v NullablePostTrackItActionsRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePostTrackItActionsRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



/*
OPERA Cloud Customer Relationship Management API

APIs to cater for Customer Relationship Management (profile) functionality in OPERA Cloud.  There are different types of profiles in OPERA Cloud, including Guest, Company, Travel Agent, Source, Group, and Contact profile types.  A profile can store and display a wide range of information about the guest, company, travel agent etc., depending upon the type of profile.  For example, a guest profile can store the guest name, address, contact information, details on billing, membership benefits, preferences and much more.  All profiles in OPERA when created are assigned a ProfileID.  This ID will be used throughout the CRM APIs.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package crm

import (
	"encoding/json"
)

// checks if the ExtendECertificateRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ExtendECertificateRequest{}

// ExtendECertificateRequest struct for ExtendECertificateRequest
type ExtendECertificateRequest struct {
	// System generated unique certificate id.
	CertificateId *int32 `json:"certificateId,omitempty"`
	// Indicates E-Certificate extended expiry date.
	ExpiryDate *string `json:"expiryDate,omitempty"`
	Links *Links `json:"links,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewExtendECertificateRequest instantiates a new ExtendECertificateRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewExtendECertificateRequest() *ExtendECertificateRequest {
	this := ExtendECertificateRequest{}
	return &this
}

// NewExtendECertificateRequestWithDefaults instantiates a new ExtendECertificateRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewExtendECertificateRequestWithDefaults() *ExtendECertificateRequest {
	this := ExtendECertificateRequest{}
	return &this
}

// GetCertificateId returns the CertificateId field value if set, zero value otherwise.
func (o *ExtendECertificateRequest) GetCertificateId() int32 {
	if o == nil || IsNil(o.CertificateId) {
		var ret int32
		return ret
	}
	return *o.CertificateId
}

// GetCertificateIdOk returns a tuple with the CertificateId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ExtendECertificateRequest) GetCertificateIdOk() (*int32, bool) {
	if o == nil || IsNil(o.CertificateId) {
		return nil, false
	}
	return o.CertificateId, true
}

// HasCertificateId returns a boolean if a field has been set.
func (o *ExtendECertificateRequest) HasCertificateId() bool {
	if o != nil && !IsNil(o.CertificateId) {
		return true
	}

	return false
}

// SetCertificateId gets a reference to the given int32 and assigns it to the CertificateId field.
func (o *ExtendECertificateRequest) SetCertificateId(v int32) {
	o.CertificateId = &v
}

// GetExpiryDate returns the ExpiryDate field value if set, zero value otherwise.
func (o *ExtendECertificateRequest) GetExpiryDate() string {
	if o == nil || IsNil(o.ExpiryDate) {
		var ret string
		return ret
	}
	return *o.ExpiryDate
}

// GetExpiryDateOk returns a tuple with the ExpiryDate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ExtendECertificateRequest) GetExpiryDateOk() (*string, bool) {
	if o == nil || IsNil(o.ExpiryDate) {
		return nil, false
	}
	return o.ExpiryDate, true
}

// HasExpiryDate returns a boolean if a field has been set.
func (o *ExtendECertificateRequest) HasExpiryDate() bool {
	if o != nil && !IsNil(o.ExpiryDate) {
		return true
	}

	return false
}

// SetExpiryDate gets a reference to the given string and assigns it to the ExpiryDate field.
func (o *ExtendECertificateRequest) SetExpiryDate(v string) {
	o.ExpiryDate = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *ExtendECertificateRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ExtendECertificateRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *ExtendECertificateRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *ExtendECertificateRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *ExtendECertificateRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ExtendECertificateRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *ExtendECertificateRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *ExtendECertificateRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o ExtendECertificateRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ExtendECertificateRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.CertificateId) {
		toSerialize["certificateId"] = o.CertificateId
	}
	if !IsNil(o.ExpiryDate) {
		toSerialize["expiryDate"] = o.ExpiryDate
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableExtendECertificateRequest struct {
	value *ExtendECertificateRequest
	isSet bool
}

func (v NullableExtendECertificateRequest) Get() *ExtendECertificateRequest {
	return v.value
}

func (v *NullableExtendECertificateRequest) Set(val *ExtendECertificateRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableExtendECertificateRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableExtendECertificateRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableExtendECertificateRequest(val *ExtendECertificateRequest) *NullableExtendECertificateRequest {
	return &NullableExtendECertificateRequest{value: val, isSet: true}
}

func (v NullableExtendECertificateRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableExtendECertificateRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



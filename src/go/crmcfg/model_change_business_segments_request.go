/*
OPERA Cloud CRM Configuration API

APIs for Customer Relationship Management (profile) configuration, such as creating preferences, or address types.  It also includes Membership Configuration, where you can retrieve membership levels that are configured for a property, or create new membership enrollment codes.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package crmcfg

import (
	"encoding/json"
)

// checks if the ChangeBusinessSegmentsRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ChangeBusinessSegmentsRequest{}

// ChangeBusinessSegmentsRequest struct for ChangeBusinessSegmentsRequest
type ChangeBusinessSegmentsRequest struct {
	BusinessSegments *BusinessSegmentsType `json:"businessSegments,omitempty"`
	Links *Links `json:"links,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewChangeBusinessSegmentsRequest instantiates a new ChangeBusinessSegmentsRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewChangeBusinessSegmentsRequest() *ChangeBusinessSegmentsRequest {
	this := ChangeBusinessSegmentsRequest{}
	return &this
}

// NewChangeBusinessSegmentsRequestWithDefaults instantiates a new ChangeBusinessSegmentsRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewChangeBusinessSegmentsRequestWithDefaults() *ChangeBusinessSegmentsRequest {
	this := ChangeBusinessSegmentsRequest{}
	return &this
}

// GetBusinessSegments returns the BusinessSegments field value if set, zero value otherwise.
func (o *ChangeBusinessSegmentsRequest) GetBusinessSegments() BusinessSegmentsType {
	if o == nil || IsNil(o.BusinessSegments) {
		var ret BusinessSegmentsType
		return ret
	}
	return *o.BusinessSegments
}

// GetBusinessSegmentsOk returns a tuple with the BusinessSegments field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeBusinessSegmentsRequest) GetBusinessSegmentsOk() (*BusinessSegmentsType, bool) {
	if o == nil || IsNil(o.BusinessSegments) {
		return nil, false
	}
	return o.BusinessSegments, true
}

// HasBusinessSegments returns a boolean if a field has been set.
func (o *ChangeBusinessSegmentsRequest) HasBusinessSegments() bool {
	if o != nil && !IsNil(o.BusinessSegments) {
		return true
	}

	return false
}

// SetBusinessSegments gets a reference to the given BusinessSegmentsType and assigns it to the BusinessSegments field.
func (o *ChangeBusinessSegmentsRequest) SetBusinessSegments(v BusinessSegmentsType) {
	o.BusinessSegments = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *ChangeBusinessSegmentsRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeBusinessSegmentsRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *ChangeBusinessSegmentsRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *ChangeBusinessSegmentsRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *ChangeBusinessSegmentsRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeBusinessSegmentsRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *ChangeBusinessSegmentsRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *ChangeBusinessSegmentsRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o ChangeBusinessSegmentsRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ChangeBusinessSegmentsRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.BusinessSegments) {
		toSerialize["businessSegments"] = o.BusinessSegments
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableChangeBusinessSegmentsRequest struct {
	value *ChangeBusinessSegmentsRequest
	isSet bool
}

func (v NullableChangeBusinessSegmentsRequest) Get() *ChangeBusinessSegmentsRequest {
	return v.value
}

func (v *NullableChangeBusinessSegmentsRequest) Set(val *ChangeBusinessSegmentsRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableChangeBusinessSegmentsRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableChangeBusinessSegmentsRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableChangeBusinessSegmentsRequest(val *ChangeBusinessSegmentsRequest) *NullableChangeBusinessSegmentsRequest {
	return &NullableChangeBusinessSegmentsRequest{value: val, isSet: true}
}

func (v NullableChangeBusinessSegmentsRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableChangeBusinessSegmentsRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



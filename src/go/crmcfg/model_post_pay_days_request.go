/*
OPERA Cloud CRM Configuration API

APIs for Customer Relationship Management (profile) configuration, such as creating preferences, or address types.  It also includes Membership Configuration, where you can retrieve membership levels that are configured for a property, or create new membership enrollment codes.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package crmcfg

import (
	"encoding/json"
)

// checks if the PostPayDaysRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PostPayDaysRequest{}

// PostPayDaysRequest struct for PostPayDaysRequest
type PostPayDaysRequest struct {
	Links *Links `json:"links,omitempty"`
	PayDays *PayDaysType `json:"payDays,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewPostPayDaysRequest instantiates a new PostPayDaysRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPostPayDaysRequest() *PostPayDaysRequest {
	this := PostPayDaysRequest{}
	return &this
}

// NewPostPayDaysRequestWithDefaults instantiates a new PostPayDaysRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPostPayDaysRequestWithDefaults() *PostPayDaysRequest {
	this := PostPayDaysRequest{}
	return &this
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PostPayDaysRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostPayDaysRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PostPayDaysRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *PostPayDaysRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetPayDays returns the PayDays field value if set, zero value otherwise.
func (o *PostPayDaysRequest) GetPayDays() PayDaysType {
	if o == nil || IsNil(o.PayDays) {
		var ret PayDaysType
		return ret
	}
	return *o.PayDays
}

// GetPayDaysOk returns a tuple with the PayDays field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostPayDaysRequest) GetPayDaysOk() (*PayDaysType, bool) {
	if o == nil || IsNil(o.PayDays) {
		return nil, false
	}
	return o.PayDays, true
}

// HasPayDays returns a boolean if a field has been set.
func (o *PostPayDaysRequest) HasPayDays() bool {
	if o != nil && !IsNil(o.PayDays) {
		return true
	}

	return false
}

// SetPayDays gets a reference to the given PayDaysType and assigns it to the PayDays field.
func (o *PostPayDaysRequest) SetPayDays(v PayDaysType) {
	o.PayDays = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PostPayDaysRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostPayDaysRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PostPayDaysRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *PostPayDaysRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o PostPayDaysRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PostPayDaysRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.PayDays) {
		toSerialize["payDays"] = o.PayDays
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePostPayDaysRequest struct {
	value *PostPayDaysRequest
	isSet bool
}

func (v NullablePostPayDaysRequest) Get() *PostPayDaysRequest {
	return v.value
}

func (v *NullablePostPayDaysRequest) Set(val *PostPayDaysRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePostPayDaysRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePostPayDaysRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePostPayDaysRequest(val *PostPayDaysRequest) *NullablePostPayDaysRequest {
	return &NullablePostPayDaysRequest{value: val, isSet: true}
}

func (v NullablePostPayDaysRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePostPayDaysRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



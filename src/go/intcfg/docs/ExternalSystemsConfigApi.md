...
> FetchedInterfaceRights GetInterfaceRights(ctx).Authorization(authorization).XAppKey(xAppKey).XHotelid(xHotelid).HotelId(hotelId).Id(id).IdExtension(idExtension).IdContext(idContext).Type_(type_).XExternalsystem(xExternalsystem).AcceptLanguage(acceptLanguage).Execute()
...
func main() {
    ...
    // response from `GetInterfaceRights`: FetchedInterfaceRights
...
[**FetchedInterfaceRights**](FetchedInterfaceRights.md)

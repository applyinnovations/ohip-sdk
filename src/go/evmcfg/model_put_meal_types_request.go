/*
OPERA Cloud Event Configuration API

This API caters for Event Configuration in OPERA Cloud. <br /><There are operations to post, update, fetch and delete codes such as item inventory, function spaces, menu items and many more.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package evmcfg

import (
	"encoding/json"
)

// checks if the PutMealTypesRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PutMealTypesRequest{}

// PutMealTypesRequest struct for PutMealTypesRequest
type PutMealTypesRequest struct {
	Links *Links `json:"links,omitempty"`
	MealTypes *MealTypesType `json:"mealTypes,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewPutMealTypesRequest instantiates a new PutMealTypesRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPutMealTypesRequest() *PutMealTypesRequest {
	this := PutMealTypesRequest{}
	return &this
}

// NewPutMealTypesRequestWithDefaults instantiates a new PutMealTypesRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPutMealTypesRequestWithDefaults() *PutMealTypesRequest {
	this := PutMealTypesRequest{}
	return &this
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PutMealTypesRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutMealTypesRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PutMealTypesRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *PutMealTypesRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetMealTypes returns the MealTypes field value if set, zero value otherwise.
func (o *PutMealTypesRequest) GetMealTypes() MealTypesType {
	if o == nil || IsNil(o.MealTypes) {
		var ret MealTypesType
		return ret
	}
	return *o.MealTypes
}

// GetMealTypesOk returns a tuple with the MealTypes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutMealTypesRequest) GetMealTypesOk() (*MealTypesType, bool) {
	if o == nil || IsNil(o.MealTypes) {
		return nil, false
	}
	return o.MealTypes, true
}

// HasMealTypes returns a boolean if a field has been set.
func (o *PutMealTypesRequest) HasMealTypes() bool {
	if o != nil && !IsNil(o.MealTypes) {
		return true
	}

	return false
}

// SetMealTypes gets a reference to the given MealTypesType and assigns it to the MealTypes field.
func (o *PutMealTypesRequest) SetMealTypes(v MealTypesType) {
	o.MealTypes = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PutMealTypesRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutMealTypesRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PutMealTypesRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *PutMealTypesRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o PutMealTypesRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PutMealTypesRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.MealTypes) {
		toSerialize["mealTypes"] = o.MealTypes
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePutMealTypesRequest struct {
	value *PutMealTypesRequest
	isSet bool
}

func (v NullablePutMealTypesRequest) Get() *PutMealTypesRequest {
	return v.value
}

func (v *NullablePutMealTypesRequest) Set(val *PutMealTypesRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePutMealTypesRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePutMealTypesRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePutMealTypesRequest(val *PutMealTypesRequest) *NullablePutMealTypesRequest {
	return &NullablePutMealTypesRequest{value: val, isSet: true}
}

func (v NullablePutMealTypesRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePutMealTypesRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



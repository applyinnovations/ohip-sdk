/*
OPERA Cloud Event Configuration API

This API caters for Event Configuration in OPERA Cloud. <br /><There are operations to post, update, fetch and delete codes such as item inventory, function spaces, menu items and many more.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package evmcfg

import (
	"encoding/json"
)

// checks if the CateringRentalCodesToBeChanged type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CateringRentalCodesToBeChanged{}

// CateringRentalCodesToBeChanged Request object which holds modified Catering Rental Codes.
type CateringRentalCodesToBeChanged struct {
	Links *Links `json:"links,omitempty"`
	RentalCodes *CateringCodesType `json:"rentalCodes,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewCateringRentalCodesToBeChanged instantiates a new CateringRentalCodesToBeChanged object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCateringRentalCodesToBeChanged() *CateringRentalCodesToBeChanged {
	this := CateringRentalCodesToBeChanged{}
	return &this
}

// NewCateringRentalCodesToBeChangedWithDefaults instantiates a new CateringRentalCodesToBeChanged object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCateringRentalCodesToBeChangedWithDefaults() *CateringRentalCodesToBeChanged {
	this := CateringRentalCodesToBeChanged{}
	return &this
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *CateringRentalCodesToBeChanged) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CateringRentalCodesToBeChanged) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *CateringRentalCodesToBeChanged) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *CateringRentalCodesToBeChanged) SetLinks(v Links) {
	o.Links = &v
}

// GetRentalCodes returns the RentalCodes field value if set, zero value otherwise.
func (o *CateringRentalCodesToBeChanged) GetRentalCodes() CateringCodesType {
	if o == nil || IsNil(o.RentalCodes) {
		var ret CateringCodesType
		return ret
	}
	return *o.RentalCodes
}

// GetRentalCodesOk returns a tuple with the RentalCodes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CateringRentalCodesToBeChanged) GetRentalCodesOk() (*CateringCodesType, bool) {
	if o == nil || IsNil(o.RentalCodes) {
		return nil, false
	}
	return o.RentalCodes, true
}

// HasRentalCodes returns a boolean if a field has been set.
func (o *CateringRentalCodesToBeChanged) HasRentalCodes() bool {
	if o != nil && !IsNil(o.RentalCodes) {
		return true
	}

	return false
}

// SetRentalCodes gets a reference to the given CateringCodesType and assigns it to the RentalCodes field.
func (o *CateringRentalCodesToBeChanged) SetRentalCodes(v CateringCodesType) {
	o.RentalCodes = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *CateringRentalCodesToBeChanged) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CateringRentalCodesToBeChanged) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *CateringRentalCodesToBeChanged) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *CateringRentalCodesToBeChanged) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o CateringRentalCodesToBeChanged) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CateringRentalCodesToBeChanged) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.RentalCodes) {
		toSerialize["rentalCodes"] = o.RentalCodes
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableCateringRentalCodesToBeChanged struct {
	value *CateringRentalCodesToBeChanged
	isSet bool
}

func (v NullableCateringRentalCodesToBeChanged) Get() *CateringRentalCodesToBeChanged {
	return v.value
}

func (v *NullableCateringRentalCodesToBeChanged) Set(val *CateringRentalCodesToBeChanged) {
	v.value = val
	v.isSet = true
}

func (v NullableCateringRentalCodesToBeChanged) IsSet() bool {
	return v.isSet
}

func (v *NullableCateringRentalCodesToBeChanged) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCateringRentalCodesToBeChanged(val *CateringRentalCodesToBeChanged) *NullableCateringRentalCodesToBeChanged {
	return &NullableCateringRentalCodesToBeChanged{value: val, isSet: true}
}

func (v NullableCateringRentalCodesToBeChanged) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCateringRentalCodesToBeChanged) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



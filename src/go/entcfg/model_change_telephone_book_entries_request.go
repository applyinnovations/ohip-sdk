/*
OPERA Cloud Enterprise Configuration API

APIs to cater for Enterprise Configuration functionality in OPERA Cloud. <br /><br In this module, you can configure a variety of options related to your properties such as their locations, facilities, and local attractions. The available options are dependant on the active controls at your property.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package entcfg

import (
	"encoding/json"
)

// checks if the ChangeTelephoneBookEntriesRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ChangeTelephoneBookEntriesRequest{}

// ChangeTelephoneBookEntriesRequest struct for ChangeTelephoneBookEntriesRequest
type ChangeTelephoneBookEntriesRequest struct {
	Links *Links `json:"links,omitempty"`
	TelephoneBookEntries *TelephoneBookEntriesType `json:"telephoneBookEntries,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewChangeTelephoneBookEntriesRequest instantiates a new ChangeTelephoneBookEntriesRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewChangeTelephoneBookEntriesRequest() *ChangeTelephoneBookEntriesRequest {
	this := ChangeTelephoneBookEntriesRequest{}
	return &this
}

// NewChangeTelephoneBookEntriesRequestWithDefaults instantiates a new ChangeTelephoneBookEntriesRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewChangeTelephoneBookEntriesRequestWithDefaults() *ChangeTelephoneBookEntriesRequest {
	this := ChangeTelephoneBookEntriesRequest{}
	return &this
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *ChangeTelephoneBookEntriesRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeTelephoneBookEntriesRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *ChangeTelephoneBookEntriesRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *ChangeTelephoneBookEntriesRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetTelephoneBookEntries returns the TelephoneBookEntries field value if set, zero value otherwise.
func (o *ChangeTelephoneBookEntriesRequest) GetTelephoneBookEntries() TelephoneBookEntriesType {
	if o == nil || IsNil(o.TelephoneBookEntries) {
		var ret TelephoneBookEntriesType
		return ret
	}
	return *o.TelephoneBookEntries
}

// GetTelephoneBookEntriesOk returns a tuple with the TelephoneBookEntries field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeTelephoneBookEntriesRequest) GetTelephoneBookEntriesOk() (*TelephoneBookEntriesType, bool) {
	if o == nil || IsNil(o.TelephoneBookEntries) {
		return nil, false
	}
	return o.TelephoneBookEntries, true
}

// HasTelephoneBookEntries returns a boolean if a field has been set.
func (o *ChangeTelephoneBookEntriesRequest) HasTelephoneBookEntries() bool {
	if o != nil && !IsNil(o.TelephoneBookEntries) {
		return true
	}

	return false
}

// SetTelephoneBookEntries gets a reference to the given TelephoneBookEntriesType and assigns it to the TelephoneBookEntries field.
func (o *ChangeTelephoneBookEntriesRequest) SetTelephoneBookEntries(v TelephoneBookEntriesType) {
	o.TelephoneBookEntries = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *ChangeTelephoneBookEntriesRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeTelephoneBookEntriesRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *ChangeTelephoneBookEntriesRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *ChangeTelephoneBookEntriesRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o ChangeTelephoneBookEntriesRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ChangeTelephoneBookEntriesRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.TelephoneBookEntries) {
		toSerialize["telephoneBookEntries"] = o.TelephoneBookEntries
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableChangeTelephoneBookEntriesRequest struct {
	value *ChangeTelephoneBookEntriesRequest
	isSet bool
}

func (v NullableChangeTelephoneBookEntriesRequest) Get() *ChangeTelephoneBookEntriesRequest {
	return v.value
}

func (v *NullableChangeTelephoneBookEntriesRequest) Set(val *ChangeTelephoneBookEntriesRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableChangeTelephoneBookEntriesRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableChangeTelephoneBookEntriesRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableChangeTelephoneBookEntriesRequest(val *ChangeTelephoneBookEntriesRequest) *NullableChangeTelephoneBookEntriesRequest {
	return &NullableChangeTelephoneBookEntriesRequest{value: val, isSet: true}
}

func (v NullableChangeTelephoneBookEntriesRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableChangeTelephoneBookEntriesRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



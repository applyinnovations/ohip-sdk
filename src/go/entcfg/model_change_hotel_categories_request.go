/*
OPERA Cloud Enterprise Configuration API

APIs to cater for Enterprise Configuration functionality in OPERA Cloud. <br /><br In this module, you can configure a variety of options related to your properties such as their locations, facilities, and local attractions. The available options are dependant on the active controls at your property.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package entcfg

import (
	"encoding/json"
)

// checks if the ChangeHotelCategoriesRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ChangeHotelCategoriesRequest{}

// ChangeHotelCategoriesRequest struct for ChangeHotelCategoriesRequest
type ChangeHotelCategoriesRequest struct {
	HotelCategories *HotelCategoriesType `json:"hotelCategories,omitempty"`
	Links *Links `json:"links,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewChangeHotelCategoriesRequest instantiates a new ChangeHotelCategoriesRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewChangeHotelCategoriesRequest() *ChangeHotelCategoriesRequest {
	this := ChangeHotelCategoriesRequest{}
	return &this
}

// NewChangeHotelCategoriesRequestWithDefaults instantiates a new ChangeHotelCategoriesRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewChangeHotelCategoriesRequestWithDefaults() *ChangeHotelCategoriesRequest {
	this := ChangeHotelCategoriesRequest{}
	return &this
}

// GetHotelCategories returns the HotelCategories field value if set, zero value otherwise.
func (o *ChangeHotelCategoriesRequest) GetHotelCategories() HotelCategoriesType {
	if o == nil || IsNil(o.HotelCategories) {
		var ret HotelCategoriesType
		return ret
	}
	return *o.HotelCategories
}

// GetHotelCategoriesOk returns a tuple with the HotelCategories field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeHotelCategoriesRequest) GetHotelCategoriesOk() (*HotelCategoriesType, bool) {
	if o == nil || IsNil(o.HotelCategories) {
		return nil, false
	}
	return o.HotelCategories, true
}

// HasHotelCategories returns a boolean if a field has been set.
func (o *ChangeHotelCategoriesRequest) HasHotelCategories() bool {
	if o != nil && !IsNil(o.HotelCategories) {
		return true
	}

	return false
}

// SetHotelCategories gets a reference to the given HotelCategoriesType and assigns it to the HotelCategories field.
func (o *ChangeHotelCategoriesRequest) SetHotelCategories(v HotelCategoriesType) {
	o.HotelCategories = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *ChangeHotelCategoriesRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeHotelCategoriesRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *ChangeHotelCategoriesRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *ChangeHotelCategoriesRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *ChangeHotelCategoriesRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeHotelCategoriesRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *ChangeHotelCategoriesRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *ChangeHotelCategoriesRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o ChangeHotelCategoriesRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ChangeHotelCategoriesRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.HotelCategories) {
		toSerialize["hotelCategories"] = o.HotelCategories
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableChangeHotelCategoriesRequest struct {
	value *ChangeHotelCategoriesRequest
	isSet bool
}

func (v NullableChangeHotelCategoriesRequest) Get() *ChangeHotelCategoriesRequest {
	return v.value
}

func (v *NullableChangeHotelCategoriesRequest) Set(val *ChangeHotelCategoriesRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableChangeHotelCategoriesRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableChangeHotelCategoriesRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableChangeHotelCategoriesRequest(val *ChangeHotelCategoriesRequest) *NullableChangeHotelCategoriesRequest {
	return &NullableChangeHotelCategoriesRequest{value: val, isSet: true}
}

func (v NullableChangeHotelCategoriesRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableChangeHotelCategoriesRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



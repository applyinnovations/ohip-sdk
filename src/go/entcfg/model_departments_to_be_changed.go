/*
OPERA Cloud Enterprise Configuration API

APIs to cater for Enterprise Configuration functionality in OPERA Cloud. <br /><br In this module, you can configure a variety of options related to your properties such as their locations, facilities, and local attractions. The available options are dependant on the active controls at your property.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package entcfg

import (
	"encoding/json"
)

// checks if the DepartmentsToBeChanged type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &DepartmentsToBeChanged{}

// DepartmentsToBeChanged Request object for updating departments.
type DepartmentsToBeChanged struct {
	Departments *DepartmentsType `json:"departments,omitempty"`
	Links *Links `json:"links,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewDepartmentsToBeChanged instantiates a new DepartmentsToBeChanged object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDepartmentsToBeChanged() *DepartmentsToBeChanged {
	this := DepartmentsToBeChanged{}
	return &this
}

// NewDepartmentsToBeChangedWithDefaults instantiates a new DepartmentsToBeChanged object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDepartmentsToBeChangedWithDefaults() *DepartmentsToBeChanged {
	this := DepartmentsToBeChanged{}
	return &this
}

// GetDepartments returns the Departments field value if set, zero value otherwise.
func (o *DepartmentsToBeChanged) GetDepartments() DepartmentsType {
	if o == nil || IsNil(o.Departments) {
		var ret DepartmentsType
		return ret
	}
	return *o.Departments
}

// GetDepartmentsOk returns a tuple with the Departments field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DepartmentsToBeChanged) GetDepartmentsOk() (*DepartmentsType, bool) {
	if o == nil || IsNil(o.Departments) {
		return nil, false
	}
	return o.Departments, true
}

// HasDepartments returns a boolean if a field has been set.
func (o *DepartmentsToBeChanged) HasDepartments() bool {
	if o != nil && !IsNil(o.Departments) {
		return true
	}

	return false
}

// SetDepartments gets a reference to the given DepartmentsType and assigns it to the Departments field.
func (o *DepartmentsToBeChanged) SetDepartments(v DepartmentsType) {
	o.Departments = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *DepartmentsToBeChanged) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DepartmentsToBeChanged) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *DepartmentsToBeChanged) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *DepartmentsToBeChanged) SetLinks(v Links) {
	o.Links = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *DepartmentsToBeChanged) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DepartmentsToBeChanged) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *DepartmentsToBeChanged) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *DepartmentsToBeChanged) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o DepartmentsToBeChanged) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o DepartmentsToBeChanged) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Departments) {
		toSerialize["departments"] = o.Departments
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableDepartmentsToBeChanged struct {
	value *DepartmentsToBeChanged
	isSet bool
}

func (v NullableDepartmentsToBeChanged) Get() *DepartmentsToBeChanged {
	return v.value
}

func (v *NullableDepartmentsToBeChanged) Set(val *DepartmentsToBeChanged) {
	v.value = val
	v.isSet = true
}

func (v NullableDepartmentsToBeChanged) IsSet() bool {
	return v.isSet
}

func (v *NullableDepartmentsToBeChanged) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDepartmentsToBeChanged(val *DepartmentsToBeChanged) *NullableDepartmentsToBeChanged {
	return &NullableDepartmentsToBeChanged{value: val, isSet: true}
}

func (v NullableDepartmentsToBeChanged) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDepartmentsToBeChanged) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



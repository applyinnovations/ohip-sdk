/*
OPERA Cloud Enterprise Configuration API

APIs to cater for Enterprise Configuration functionality in OPERA Cloud. <br /><br In this module, you can configure a variety of options related to your properties such as their locations, facilities, and local attractions. The available options are dependant on the active controls at your property.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package entcfg

import (
	"encoding/json"
)

// checks if the ChangeCorporateDepartmentCodesRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ChangeCorporateDepartmentCodesRequest{}

// ChangeCorporateDepartmentCodesRequest struct for ChangeCorporateDepartmentCodesRequest
type ChangeCorporateDepartmentCodesRequest struct {
	CorporateDepartmentCodes *CorporateDepartmentCodesType `json:"corporateDepartmentCodes,omitempty"`
	Links *Links `json:"links,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewChangeCorporateDepartmentCodesRequest instantiates a new ChangeCorporateDepartmentCodesRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewChangeCorporateDepartmentCodesRequest() *ChangeCorporateDepartmentCodesRequest {
	this := ChangeCorporateDepartmentCodesRequest{}
	return &this
}

// NewChangeCorporateDepartmentCodesRequestWithDefaults instantiates a new ChangeCorporateDepartmentCodesRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewChangeCorporateDepartmentCodesRequestWithDefaults() *ChangeCorporateDepartmentCodesRequest {
	this := ChangeCorporateDepartmentCodesRequest{}
	return &this
}

// GetCorporateDepartmentCodes returns the CorporateDepartmentCodes field value if set, zero value otherwise.
func (o *ChangeCorporateDepartmentCodesRequest) GetCorporateDepartmentCodes() CorporateDepartmentCodesType {
	if o == nil || IsNil(o.CorporateDepartmentCodes) {
		var ret CorporateDepartmentCodesType
		return ret
	}
	return *o.CorporateDepartmentCodes
}

// GetCorporateDepartmentCodesOk returns a tuple with the CorporateDepartmentCodes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeCorporateDepartmentCodesRequest) GetCorporateDepartmentCodesOk() (*CorporateDepartmentCodesType, bool) {
	if o == nil || IsNil(o.CorporateDepartmentCodes) {
		return nil, false
	}
	return o.CorporateDepartmentCodes, true
}

// HasCorporateDepartmentCodes returns a boolean if a field has been set.
func (o *ChangeCorporateDepartmentCodesRequest) HasCorporateDepartmentCodes() bool {
	if o != nil && !IsNil(o.CorporateDepartmentCodes) {
		return true
	}

	return false
}

// SetCorporateDepartmentCodes gets a reference to the given CorporateDepartmentCodesType and assigns it to the CorporateDepartmentCodes field.
func (o *ChangeCorporateDepartmentCodesRequest) SetCorporateDepartmentCodes(v CorporateDepartmentCodesType) {
	o.CorporateDepartmentCodes = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *ChangeCorporateDepartmentCodesRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeCorporateDepartmentCodesRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *ChangeCorporateDepartmentCodesRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *ChangeCorporateDepartmentCodesRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *ChangeCorporateDepartmentCodesRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ChangeCorporateDepartmentCodesRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *ChangeCorporateDepartmentCodesRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *ChangeCorporateDepartmentCodesRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o ChangeCorporateDepartmentCodesRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ChangeCorporateDepartmentCodesRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.CorporateDepartmentCodes) {
		toSerialize["corporateDepartmentCodes"] = o.CorporateDepartmentCodes
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableChangeCorporateDepartmentCodesRequest struct {
	value *ChangeCorporateDepartmentCodesRequest
	isSet bool
}

func (v NullableChangeCorporateDepartmentCodesRequest) Get() *ChangeCorporateDepartmentCodesRequest {
	return v.value
}

func (v *NullableChangeCorporateDepartmentCodesRequest) Set(val *ChangeCorporateDepartmentCodesRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableChangeCorporateDepartmentCodesRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableChangeCorporateDepartmentCodesRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableChangeCorporateDepartmentCodesRequest(val *ChangeCorporateDepartmentCodesRequest) *NullableChangeCorporateDepartmentCodesRequest {
	return &NullableChangeCorporateDepartmentCodesRequest{value: val, isSet: true}
}

func (v NullableChangeCorporateDepartmentCodesRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableChangeCorporateDepartmentCodesRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



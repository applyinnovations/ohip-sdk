/*
OPERA Cloud Enterprise Configuration API

APIs to cater for Enterprise Configuration functionality in OPERA Cloud. <br /><br In this module, you can configure a variety of options related to your properties such as their locations, facilities, and local attractions. The available options are dependant on the active controls at your property.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package entcfg

import (
	"encoding/json"
)

// checks if the RegionsCodeToBeChanged type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &RegionsCodeToBeChanged{}

// RegionsCodeToBeChanged Request object for changing Regions Code.
type RegionsCodeToBeChanged struct {
	Links *Links `json:"links,omitempty"`
	RegionsCode *RegionsCodeType `json:"regionsCode,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewRegionsCodeToBeChanged instantiates a new RegionsCodeToBeChanged object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRegionsCodeToBeChanged() *RegionsCodeToBeChanged {
	this := RegionsCodeToBeChanged{}
	return &this
}

// NewRegionsCodeToBeChangedWithDefaults instantiates a new RegionsCodeToBeChanged object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRegionsCodeToBeChangedWithDefaults() *RegionsCodeToBeChanged {
	this := RegionsCodeToBeChanged{}
	return &this
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *RegionsCodeToBeChanged) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RegionsCodeToBeChanged) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *RegionsCodeToBeChanged) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *RegionsCodeToBeChanged) SetLinks(v Links) {
	o.Links = &v
}

// GetRegionsCode returns the RegionsCode field value if set, zero value otherwise.
func (o *RegionsCodeToBeChanged) GetRegionsCode() RegionsCodeType {
	if o == nil || IsNil(o.RegionsCode) {
		var ret RegionsCodeType
		return ret
	}
	return *o.RegionsCode
}

// GetRegionsCodeOk returns a tuple with the RegionsCode field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RegionsCodeToBeChanged) GetRegionsCodeOk() (*RegionsCodeType, bool) {
	if o == nil || IsNil(o.RegionsCode) {
		return nil, false
	}
	return o.RegionsCode, true
}

// HasRegionsCode returns a boolean if a field has been set.
func (o *RegionsCodeToBeChanged) HasRegionsCode() bool {
	if o != nil && !IsNil(o.RegionsCode) {
		return true
	}

	return false
}

// SetRegionsCode gets a reference to the given RegionsCodeType and assigns it to the RegionsCode field.
func (o *RegionsCodeToBeChanged) SetRegionsCode(v RegionsCodeType) {
	o.RegionsCode = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *RegionsCodeToBeChanged) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RegionsCodeToBeChanged) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *RegionsCodeToBeChanged) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *RegionsCodeToBeChanged) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o RegionsCodeToBeChanged) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o RegionsCodeToBeChanged) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.RegionsCode) {
		toSerialize["regionsCode"] = o.RegionsCode
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableRegionsCodeToBeChanged struct {
	value *RegionsCodeToBeChanged
	isSet bool
}

func (v NullableRegionsCodeToBeChanged) Get() *RegionsCodeToBeChanged {
	return v.value
}

func (v *NullableRegionsCodeToBeChanged) Set(val *RegionsCodeToBeChanged) {
	v.value = val
	v.isSet = true
}

func (v NullableRegionsCodeToBeChanged) IsSet() bool {
	return v.isSet
}

func (v *NullableRegionsCodeToBeChanged) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRegionsCodeToBeChanged(val *RegionsCodeToBeChanged) *NullableRegionsCodeToBeChanged {
	return &NullableRegionsCodeToBeChanged{value: val, isSet: true}
}

func (v NullableRegionsCodeToBeChanged) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRegionsCodeToBeChanged) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



/*
OPERA Cloud Accounts Receivables API

APIs to cater for Accounts Receivables functionality in OPERA Cloud. <br /><br The OPERA Cloud Accounts Receivable module enables you to manage debtors' accounts, invoices, and remittance.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package ars

import (
	"encoding/json"
)

// checks if the PutAccountRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PutAccountRequest{}

// PutAccountRequest struct for PutAccountRequest
type PutAccountRequest struct {
	AccountDetails *ARAccountType `json:"accountDetails,omitempty"`
	Links *Links `json:"links,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewPutAccountRequest instantiates a new PutAccountRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPutAccountRequest() *PutAccountRequest {
	this := PutAccountRequest{}
	return &this
}

// NewPutAccountRequestWithDefaults instantiates a new PutAccountRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPutAccountRequestWithDefaults() *PutAccountRequest {
	this := PutAccountRequest{}
	return &this
}

// GetAccountDetails returns the AccountDetails field value if set, zero value otherwise.
func (o *PutAccountRequest) GetAccountDetails() ARAccountType {
	if o == nil || IsNil(o.AccountDetails) {
		var ret ARAccountType
		return ret
	}
	return *o.AccountDetails
}

// GetAccountDetailsOk returns a tuple with the AccountDetails field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutAccountRequest) GetAccountDetailsOk() (*ARAccountType, bool) {
	if o == nil || IsNil(o.AccountDetails) {
		return nil, false
	}
	return o.AccountDetails, true
}

// HasAccountDetails returns a boolean if a field has been set.
func (o *PutAccountRequest) HasAccountDetails() bool {
	if o != nil && !IsNil(o.AccountDetails) {
		return true
	}

	return false
}

// SetAccountDetails gets a reference to the given ARAccountType and assigns it to the AccountDetails field.
func (o *PutAccountRequest) SetAccountDetails(v ARAccountType) {
	o.AccountDetails = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PutAccountRequest) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutAccountRequest) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PutAccountRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *PutAccountRequest) SetLinks(v Links) {
	o.Links = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PutAccountRequest) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutAccountRequest) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PutAccountRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *PutAccountRequest) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o PutAccountRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PutAccountRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.AccountDetails) {
		toSerialize["accountDetails"] = o.AccountDetails
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePutAccountRequest struct {
	value *PutAccountRequest
	isSet bool
}

func (v NullablePutAccountRequest) Get() *PutAccountRequest {
	return v.value
}

func (v *NullablePutAccountRequest) Set(val *PutAccountRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePutAccountRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePutAccountRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePutAccountRequest(val *PutAccountRequest) *NullablePutAccountRequest {
	return &NullablePutAccountRequest{value: val, isSet: true}
}

func (v NullablePutAccountRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePutAccountRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



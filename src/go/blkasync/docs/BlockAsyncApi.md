...
> BlockAllocationSummary GetBlockAllocationSummary(ctx, requestId, hotelId, extSystemCode).Authorization(authorization).XAppKey(xAppKey).XHotelid(xHotelid).AcceptLanguage(acceptLanguage).Execute()
...
func main() {
    ...
    // response from `GetBlockAllocationSummary`: BlockAllocationSummary
...
[**BlockAllocationSummary**](BlockAllocationSummary.md)

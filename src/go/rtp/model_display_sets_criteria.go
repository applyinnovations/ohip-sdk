/*
OPERA Cloud Rate API

APIs to cater for Rate Availability functionality in OPERA Cloud. <br /><br /> Rate Management provides all the tools you need to effectively define and manage the rate structures for a property in OPERA Cloud. Some of the things you can do include creating and managing rate codes, rate classes, rate categories, display sets, rate strategies, as well as managing promotion groups and codes.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package rtp

import (
	"encoding/json"
)

// checks if the DisplaySetsCriteria type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &DisplaySetsCriteria{}

// DisplaySetsCriteria Request object for creating Display Sets.
type DisplaySetsCriteria struct {
	DisplaySets *DisplaySetsType `json:"displaySets,omitempty"`
	Links *Links `json:"links,omitempty"`
	Warnings *WarningsType `json:"warnings,omitempty"`
}

// NewDisplaySetsCriteria instantiates a new DisplaySetsCriteria object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDisplaySetsCriteria() *DisplaySetsCriteria {
	this := DisplaySetsCriteria{}
	return &this
}

// NewDisplaySetsCriteriaWithDefaults instantiates a new DisplaySetsCriteria object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDisplaySetsCriteriaWithDefaults() *DisplaySetsCriteria {
	this := DisplaySetsCriteria{}
	return &this
}

// GetDisplaySets returns the DisplaySets field value if set, zero value otherwise.
func (o *DisplaySetsCriteria) GetDisplaySets() DisplaySetsType {
	if o == nil || IsNil(o.DisplaySets) {
		var ret DisplaySetsType
		return ret
	}
	return *o.DisplaySets
}

// GetDisplaySetsOk returns a tuple with the DisplaySets field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DisplaySetsCriteria) GetDisplaySetsOk() (*DisplaySetsType, bool) {
	if o == nil || IsNil(o.DisplaySets) {
		return nil, false
	}
	return o.DisplaySets, true
}

// HasDisplaySets returns a boolean if a field has been set.
func (o *DisplaySetsCriteria) HasDisplaySets() bool {
	if o != nil && !IsNil(o.DisplaySets) {
		return true
	}

	return false
}

// SetDisplaySets gets a reference to the given DisplaySetsType and assigns it to the DisplaySets field.
func (o *DisplaySetsCriteria) SetDisplaySets(v DisplaySetsType) {
	o.DisplaySets = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *DisplaySetsCriteria) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DisplaySetsCriteria) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *DisplaySetsCriteria) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *DisplaySetsCriteria) SetLinks(v Links) {
	o.Links = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *DisplaySetsCriteria) GetWarnings() WarningsType {
	if o == nil || IsNil(o.Warnings) {
		var ret WarningsType
		return ret
	}
	return *o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DisplaySetsCriteria) GetWarningsOk() (*WarningsType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *DisplaySetsCriteria) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given WarningsType and assigns it to the Warnings field.
func (o *DisplaySetsCriteria) SetWarnings(v WarningsType) {
	o.Warnings = &v
}

func (o DisplaySetsCriteria) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o DisplaySetsCriteria) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.DisplaySets) {
		toSerialize["displaySets"] = o.DisplaySets
	}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullableDisplaySetsCriteria struct {
	value *DisplaySetsCriteria
	isSet bool
}

func (v NullableDisplaySetsCriteria) Get() *DisplaySetsCriteria {
	return v.value
}

func (v *NullableDisplaySetsCriteria) Set(val *DisplaySetsCriteria) {
	v.value = val
	v.isSet = true
}

func (v NullableDisplaySetsCriteria) IsSet() bool {
	return v.isSet
}

func (v *NullableDisplaySetsCriteria) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDisplaySetsCriteria(val *DisplaySetsCriteria) *NullableDisplaySetsCriteria {
	return &NullableDisplaySetsCriteria{value: val, isSet: true}
}

func (v NullableDisplaySetsCriteria) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDisplaySetsCriteria) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



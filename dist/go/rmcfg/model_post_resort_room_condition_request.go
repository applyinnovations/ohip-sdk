/*
OPERA Cloud Room Configuration API

APIs to cater for room configuration, such as configuring room types, room Classes, creating new room features, or updating housekeeping room maintenance reasons.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package rmcfg

import (
	"encoding/json"
)

// checks if the PostResortRoomConditionRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PostResortRoomConditionRequest{}

// PostResortRoomConditionRequest struct for PostResortRoomConditionRequest
type PostResortRoomConditionRequest struct {
	Links []InstanceLink `json:"links,omitempty"`
	RoomCondition *RoomCondtionType `json:"roomCondition,omitempty"`
	// Used in conjunction with the Success element to define a business error.
	Warnings []WarningType `json:"warnings,omitempty"`
}

// NewPostResortRoomConditionRequest instantiates a new PostResortRoomConditionRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPostResortRoomConditionRequest() *PostResortRoomConditionRequest {
	this := PostResortRoomConditionRequest{}
	return &this
}

// NewPostResortRoomConditionRequestWithDefaults instantiates a new PostResortRoomConditionRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPostResortRoomConditionRequestWithDefaults() *PostResortRoomConditionRequest {
	this := PostResortRoomConditionRequest{}
	return &this
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PostResortRoomConditionRequest) GetLinks() []InstanceLink {
	if o == nil || IsNil(o.Links) {
		var ret []InstanceLink
		return ret
	}
	return o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostResortRoomConditionRequest) GetLinksOk() ([]InstanceLink, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PostResortRoomConditionRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given []InstanceLink and assigns it to the Links field.
func (o *PostResortRoomConditionRequest) SetLinks(v []InstanceLink) {
	o.Links = v
}

// GetRoomCondition returns the RoomCondition field value if set, zero value otherwise.
func (o *PostResortRoomConditionRequest) GetRoomCondition() RoomCondtionType {
	if o == nil || IsNil(o.RoomCondition) {
		var ret RoomCondtionType
		return ret
	}
	return *o.RoomCondition
}

// GetRoomConditionOk returns a tuple with the RoomCondition field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostResortRoomConditionRequest) GetRoomConditionOk() (*RoomCondtionType, bool) {
	if o == nil || IsNil(o.RoomCondition) {
		return nil, false
	}
	return o.RoomCondition, true
}

// HasRoomCondition returns a boolean if a field has been set.
func (o *PostResortRoomConditionRequest) HasRoomCondition() bool {
	if o != nil && !IsNil(o.RoomCondition) {
		return true
	}

	return false
}

// SetRoomCondition gets a reference to the given RoomCondtionType and assigns it to the RoomCondition field.
func (o *PostResortRoomConditionRequest) SetRoomCondition(v RoomCondtionType) {
	o.RoomCondition = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PostResortRoomConditionRequest) GetWarnings() []WarningType {
	if o == nil || IsNil(o.Warnings) {
		var ret []WarningType
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostResortRoomConditionRequest) GetWarningsOk() ([]WarningType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PostResortRoomConditionRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []WarningType and assigns it to the Warnings field.
func (o *PostResortRoomConditionRequest) SetWarnings(v []WarningType) {
	o.Warnings = v
}

func (o PostResortRoomConditionRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PostResortRoomConditionRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.RoomCondition) {
		toSerialize["roomCondition"] = o.RoomCondition
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePostResortRoomConditionRequest struct {
	value *PostResortRoomConditionRequest
	isSet bool
}

func (v NullablePostResortRoomConditionRequest) Get() *PostResortRoomConditionRequest {
	return v.value
}

func (v *NullablePostResortRoomConditionRequest) Set(val *PostResortRoomConditionRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePostResortRoomConditionRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePostResortRoomConditionRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePostResortRoomConditionRequest(val *PostResortRoomConditionRequest) *NullablePostResortRoomConditionRequest {
	return &NullablePostResortRoomConditionRequest{value: val, isSet: true}
}

func (v NullablePostResortRoomConditionRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePostResortRoomConditionRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



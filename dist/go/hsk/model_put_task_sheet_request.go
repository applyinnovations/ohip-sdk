/*
OPERA Cloud Housekeeping Service API

APIs to cater for Housekeeping functionality in OPERA Cloud. <br /><br />Housekeeping enables you to schedule daily room cleaning, maintenance, and housekeeping staff activities. It provides information on room status, out of order/out of service rooms, and forecasting.<br /><br /> Compatible with OPERA Cloud release 23.2.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>

API version: 23.2
Contact: hospitality_apis_ww_grp@oracle.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package hsk

import (
	"encoding/json"
)

// checks if the PutTaskSheetRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PutTaskSheetRequest{}

// PutTaskSheetRequest struct for PutTaskSheetRequest
type PutTaskSheetRequest struct {
	Links []InstanceLink `json:"links,omitempty"`
	TaskSheetDetails *HousekeepingTaskType `json:"taskSheetDetails,omitempty"`
	// Used in conjunction with the Success element to define a business error.
	Warnings []WarningType `json:"warnings,omitempty"`
}

// NewPutTaskSheetRequest instantiates a new PutTaskSheetRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPutTaskSheetRequest() *PutTaskSheetRequest {
	this := PutTaskSheetRequest{}
	return &this
}

// NewPutTaskSheetRequestWithDefaults instantiates a new PutTaskSheetRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPutTaskSheetRequestWithDefaults() *PutTaskSheetRequest {
	this := PutTaskSheetRequest{}
	return &this
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PutTaskSheetRequest) GetLinks() []InstanceLink {
	if o == nil || IsNil(o.Links) {
		var ret []InstanceLink
		return ret
	}
	return o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutTaskSheetRequest) GetLinksOk() ([]InstanceLink, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PutTaskSheetRequest) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given []InstanceLink and assigns it to the Links field.
func (o *PutTaskSheetRequest) SetLinks(v []InstanceLink) {
	o.Links = v
}

// GetTaskSheetDetails returns the TaskSheetDetails field value if set, zero value otherwise.
func (o *PutTaskSheetRequest) GetTaskSheetDetails() HousekeepingTaskType {
	if o == nil || IsNil(o.TaskSheetDetails) {
		var ret HousekeepingTaskType
		return ret
	}
	return *o.TaskSheetDetails
}

// GetTaskSheetDetailsOk returns a tuple with the TaskSheetDetails field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutTaskSheetRequest) GetTaskSheetDetailsOk() (*HousekeepingTaskType, bool) {
	if o == nil || IsNil(o.TaskSheetDetails) {
		return nil, false
	}
	return o.TaskSheetDetails, true
}

// HasTaskSheetDetails returns a boolean if a field has been set.
func (o *PutTaskSheetRequest) HasTaskSheetDetails() bool {
	if o != nil && !IsNil(o.TaskSheetDetails) {
		return true
	}

	return false
}

// SetTaskSheetDetails gets a reference to the given HousekeepingTaskType and assigns it to the TaskSheetDetails field.
func (o *PutTaskSheetRequest) SetTaskSheetDetails(v HousekeepingTaskType) {
	o.TaskSheetDetails = &v
}

// GetWarnings returns the Warnings field value if set, zero value otherwise.
func (o *PutTaskSheetRequest) GetWarnings() []WarningType {
	if o == nil || IsNil(o.Warnings) {
		var ret []WarningType
		return ret
	}
	return o.Warnings
}

// GetWarningsOk returns a tuple with the Warnings field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PutTaskSheetRequest) GetWarningsOk() ([]WarningType, bool) {
	if o == nil || IsNil(o.Warnings) {
		return nil, false
	}
	return o.Warnings, true
}

// HasWarnings returns a boolean if a field has been set.
func (o *PutTaskSheetRequest) HasWarnings() bool {
	if o != nil && !IsNil(o.Warnings) {
		return true
	}

	return false
}

// SetWarnings gets a reference to the given []WarningType and assigns it to the Warnings field.
func (o *PutTaskSheetRequest) SetWarnings(v []WarningType) {
	o.Warnings = v
}

func (o PutTaskSheetRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PutTaskSheetRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Links) {
		toSerialize["links"] = o.Links
	}
	if !IsNil(o.TaskSheetDetails) {
		toSerialize["taskSheetDetails"] = o.TaskSheetDetails
	}
	if !IsNil(o.Warnings) {
		toSerialize["warnings"] = o.Warnings
	}
	return toSerialize, nil
}

type NullablePutTaskSheetRequest struct {
	value *PutTaskSheetRequest
	isSet bool
}

func (v NullablePutTaskSheetRequest) Get() *PutTaskSheetRequest {
	return v.value
}

func (v *NullablePutTaskSheetRequest) Set(val *PutTaskSheetRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePutTaskSheetRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePutTaskSheetRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePutTaskSheetRequest(val *PutTaskSheetRequest) *NullablePutTaskSheetRequest {
	return &NullablePutTaskSheetRequest{value: val, isSet: true}
}

func (v NullablePutTaskSheetRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePutTaskSheetRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



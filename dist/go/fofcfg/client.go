...
import (
	...
	"time"
	"unicode/utf8"
	"github.com/applyinnovations/ohip-sdk/dist/go/oauth"
	"math"

)

const maxBackOffTimeout = 60 * 60 * 1000
const maxBackOffTimeoutToWait = 10 * 1000
const powerBackOffTimeout = 4
const baseBackOffTimeout = 100
...
// APIClient manages communication with the OPERA Cloud Front Desk Configuration API API v23.2
...
type APIClient struct {
	...
	AccountsReceivablesConfigApi *AccountsReceivablesConfigApiService

	CashieringConfigApi *CashieringConfigApiService

	ChainConfigApi *ChainConfigApiService

	CommissionConfigApi *CommissionConfigApiService

	CreditCardApi *CreditCardApiService

	HotelConfigApi *HotelConfigApiService
...
func NewAPIClient(cfg *Configuration) *APIClient {
	...
	// API Services
	c.AccountsReceivablesConfigApi = (*AccountsReceivablesConfigApiService)(&c.common)
	c.CashieringConfigApi = (*CashieringConfigApiService)(&c.common)
	c.ChainConfigApi = (*ChainConfigApiService)(&c.common)
	c.CommissionConfigApi = (*CommissionConfigApiService)(&c.common)
	c.CreditCardApi = (*CreditCardApiService)(&c.common)
	c.HotelConfigApi = (*HotelConfigApiService)(&c.common)
...
func (c *APIClient) callAPI(request *http.Request) (*http.Response, error) {
	...
	handleClientRequestAuthenticationError(c, *resp)
	if err != nil {
		return resp, err
	}
...
func (c *APIClient) prepareRequest(
	...
	if len(headerParams) > 0 {
		...
		headers["x-app-key"] = []string{c.cfg.AppKey}
...
func setTokenHeaders(c *APIClient, response oauth.OAuth2TokenResponse){
	...
	c.tokenExpiration = int64(time.Now().UnixMilli() + *response.ExpiresIn * 900)
	// Refresh token is currently not returned by oauth endpoint
...
func setBackoffTimeout(c *APIClient) {
	...
	delay := int64(math.Min(
		float64(baseBackOffTimeout) * math.Pow(powerBackOffTimeout, math.Floor(float64(c.authTries / credLength))),
...
func isAuthTokenExpired(c *APIClient) bool {
  return time.Now().UnixMilli() > c.tokenExpiration;
}

func handleClientRequestAuthenticationError(c *APIClient, response http.Response) error {

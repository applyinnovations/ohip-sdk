...
> Events GetEventsMultipleHotels(ctx).Authorization(authorization).XAppKey(xAppKey).XHotelid(xHotelid).HotelIds(hotelIds).EventStartDate(eventStartDate).EventEndDate(eventEndDate).BlockId(blockId).EventId(eventId).Category(category).Name(name).AttachedProfileId(attachedProfileId).BlockName(blockName).EventStatusCodes(eventStatusCodes).EventTypeCodes(eventTypeCodes).FunctionSpaceCodes(functionSpaceCodes).IncludeInactive(includeInactive).IncludeAlternateEvents(includeAlternateEvents).IncludeOpportunities(includeOpportunities).IncludeSubEvents(includeSubEvents).CateringEventOrderBy(cateringEventOrderBy).FetchInstructions(fetchInstructions).Limit(limit).Offset(offset).CateringOwners(cateringOwners).XExternalsystem(xExternalsystem).AcceptLanguage(acceptLanguage).Execute()
...
func main() {
    ...
    fetchInstructions := []string{"FetchInstructions_example"} // []string | Instructions to fetch details information about the selected event. The BlocksInfo fetch instructions will retain a limited amount of data for the associated block.  The Indicators instruction will provide a count. The EventResources fetch instruction if passed with eventId will return the required page number and records on it, i.e. it will return same page records which has the passed eventId. (optional)
    ...
    cateringOwners := []string{"Inner_example"} // []string | Catering Owner Codes associated with the events to be fetched. (optional)
    xExternalsystem := "xExternalsystem_example" // string | External system code. (optional)
    acceptLanguage := "acceptLanguage_example" // string | Language code (optional)
 ...
 **fetchInstructions** | **[]string** | Instructions to fetch details information about the selected event. The BlocksInfo fetch instructions will retain a limited amount of data for the associated block.  The Indicators instruction will provide a count. The EventResources fetch instruction if passed with eventId will return the required page number and records on it, i.e. it will return same page records which has the passed eventId. | 
 **limit** | **int32** | Indicates maximum number of records a Web Service should return. | [default to 200]
 **offset** | **int32** | Index or initial index of the set(page) being requested. If the index goes out of the bounds of the total set count then no data will be returned. | [default to 0]
 **cateringOwners** | **[]string** | Catering Owner Codes associated with the events to be fetched. | 
 **xExternalsystem** | **string** | External system code. | 
 **acceptLanguage** | **string** | Language code | 
...
func main() {
    ...
    spaceTypes := []string{"Inner_example"} // []string |  (optional)

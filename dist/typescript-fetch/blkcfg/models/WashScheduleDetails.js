"use strict";
/* tslint:disable */
/* eslint-disable */
/**
 * OPERA Cloud Block Configuration API
 * APIs for Block configuration, such as creating, updating, fetching and removing codes related to blocks. <br />< This might include fetching the block cancellation reasons, or creating new block refused reasons.  Wash schedules can be create, or new reservation methods could be added for a property.<br /><br /> Compatible with OPERA Cloud release 24.4.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2024 Oracle and/or its affiliates.</p>
 *
 * The version of the OpenAPI document: 24.4
 * Contact: hospitality_apis_ww_grp@oracle.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.WashScheduleDetailsToJSON = exports.WashScheduleDetailsFromJSONTyped = exports.WashScheduleDetailsFromJSON = exports.instanceOfWashScheduleDetails = void 0;
const runtime_1 = require("../runtime");
const InstanceLink_1 = require("./InstanceLink");
const WarningType_1 = require("./WarningType");
const WashCodeType_1 = require("./WashCodeType");
/**
 * Check if a given object implements the WashScheduleDetails interface.
 */
function instanceOfWashScheduleDetails(value) {
    let isInstance = true;
    return isInstance;
}
exports.instanceOfWashScheduleDetails = instanceOfWashScheduleDetails;
function WashScheduleDetailsFromJSON(json) {
    return WashScheduleDetailsFromJSONTyped(json, false);
}
exports.WashScheduleDetailsFromJSON = WashScheduleDetailsFromJSON;
function WashScheduleDetailsFromJSONTyped(json, ignoreDiscriminator) {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        'washCodes': !(0, runtime_1.exists)(json, 'washCodes') ? undefined : (json['washCodes'].map(WashCodeType_1.WashCodeTypeFromJSON)),
        'links': !(0, runtime_1.exists)(json, 'links') ? undefined : (json['links'].map(InstanceLink_1.InstanceLinkFromJSON)),
        'warnings': !(0, runtime_1.exists)(json, 'warnings') ? undefined : (json['warnings'].map(WarningType_1.WarningTypeFromJSON)),
    };
}
exports.WashScheduleDetailsFromJSONTyped = WashScheduleDetailsFromJSONTyped;
function WashScheduleDetailsToJSON(value) {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        'washCodes': value.washCodes === undefined ? undefined : (value.washCodes.map(WashCodeType_1.WashCodeTypeToJSON)),
        'links': value.links === undefined ? undefined : (value.links.map(InstanceLink_1.InstanceLinkToJSON)),
        'warnings': value.warnings === undefined ? undefined : (value.warnings.map(WarningType_1.WarningTypeToJSON)),
    };
}
exports.WashScheduleDetailsToJSON = WashScheduleDetailsToJSON;

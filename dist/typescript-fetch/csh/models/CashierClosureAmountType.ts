/* tslint:disable */
/* eslint-disable */
/**
 * OPERA Cloud Cashiering API
 * APIs to cater for Cashiering related functionality in OPERA Cloud.<br /><br /> Cashiering provides access to a guest folio, posting journals, receipt histories, currency calculations, credit card settlements, and check a guest out.<br /><br /> Compatible with OPERA Cloud release 22.3.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>
 *
 * The version of the OpenAPI document: 22.3
 * Contact: hospitality_apis_ww_grp@oracle.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
/**
 * Cashier Opening Balance and Shift Amounts type.
 * @export
 * @interface CashierClosureAmountType
 */
export interface CashierClosureAmountType {
    /**
     * Cashier shift amounts type.
     * @type {number}
     * @memberof CashierClosureAmountType
     */
    openingBalance?: number;
    /**
     * Receviced amount.
     * @type {number}
     * @memberof CashierClosureAmountType
     */
    received?: number;
    /**
     * Paid Out amount.
     * @type {number}
     * @memberof CashierClosureAmountType
     */
    paidOut?: number;
    /**
     * Expected Cashier shift amount.
     * @type {number}
     * @memberof CashierClosureAmountType
     */
    expectedShiftBalance?: number;
    /**
     * Actual Cashier shift amount.
     * @type {number}
     * @memberof CashierClosureAmountType
     */
    actualShiftBalance?: number;
}

/**
 * Check if a given object implements the CashierClosureAmountType interface.
 */
export function instanceOfCashierClosureAmountType(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function CashierClosureAmountTypeFromJSON(json: any): CashierClosureAmountType {
    return CashierClosureAmountTypeFromJSONTyped(json, false);
}

export function CashierClosureAmountTypeFromJSONTyped(json: any, ignoreDiscriminator: boolean): CashierClosureAmountType {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'openingBalance': !exists(json, 'openingBalance') ? undefined : json['openingBalance'],
        'received': !exists(json, 'received') ? undefined : json['received'],
        'paidOut': !exists(json, 'paidOut') ? undefined : json['paidOut'],
        'expectedShiftBalance': !exists(json, 'expectedShiftBalance') ? undefined : json['expectedShiftBalance'],
        'actualShiftBalance': !exists(json, 'actualShiftBalance') ? undefined : json['actualShiftBalance'],
    };
}

export function CashierClosureAmountTypeToJSON(value?: CashierClosureAmountType | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'openingBalance': value.openingBalance,
        'received': value.received,
        'paidOut': value.paidOut,
        'expectedShiftBalance': value.expectedShiftBalance,
        'actualShiftBalance': value.actualShiftBalance,
    };
}


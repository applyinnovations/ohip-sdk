/* tslint:disable */
/* eslint-disable */
/**
 * OPERA Cloud Cashiering API
 * APIs to cater for Cashiering related functionality in OPERA Cloud.<br /><br /> Cashiering provides access to a guest folio, posting journals, receipt histories, currency calculations, credit card settlements, and check a guest out.<br /><br /> Compatible with OPERA Cloud release 22.3.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>
 *
 * The version of the OpenAPI document: 22.3
 * Contact: hospitality_apis_ww_grp@oracle.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import type { InstanceLink } from './InstanceLink';
import {
    InstanceLinkFromJSON,
    InstanceLinkFromJSONTyped,
    InstanceLinkToJSON,
} from './InstanceLink';
import type { WarningType } from './WarningType';
import {
    WarningTypeFromJSON,
    WarningTypeFromJSONTyped,
    WarningTypeToJSON,
} from './WarningType';

/**
 * The response object to send back the fiscal service response to opera.
 * @export
 * @interface ProcessFiscalRetryStatus
 */
export interface ProcessFiscalRetryStatus {
    /**
     * Fiscal Folio sequence ID stored in queue table.
     * @type {number}
     * @memberof ProcessFiscalRetryStatus
     */
    folioSeqId?: number;
    /**
     * Indicating if it is last trial for fiscal printing.
     * @type {boolean}
     * @memberof ProcessFiscalRetryStatus
     */
    isLastRetry?: boolean;
    /**
     * 
     * @type {Array<InstanceLink>}
     * @memberof ProcessFiscalRetryStatus
     */
    links?: Array<InstanceLink>;
    /**
     * Used in conjunction with the Success element to define a business error.
     * @type {Array<WarningType>}
     * @memberof ProcessFiscalRetryStatus
     */
    warnings?: Array<WarningType>;
}

/**
 * Check if a given object implements the ProcessFiscalRetryStatus interface.
 */
export function instanceOfProcessFiscalRetryStatus(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function ProcessFiscalRetryStatusFromJSON(json: any): ProcessFiscalRetryStatus {
    return ProcessFiscalRetryStatusFromJSONTyped(json, false);
}

export function ProcessFiscalRetryStatusFromJSONTyped(json: any, ignoreDiscriminator: boolean): ProcessFiscalRetryStatus {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'folioSeqId': !exists(json, 'folioSeqId') ? undefined : json['folioSeqId'],
        'isLastRetry': !exists(json, 'isLastRetry') ? undefined : json['isLastRetry'],
        'links': !exists(json, 'links') ? undefined : ((json['links'] as Array<any>).map(InstanceLinkFromJSON)),
        'warnings': !exists(json, 'warnings') ? undefined : ((json['warnings'] as Array<any>).map(WarningTypeFromJSON)),
    };
}

export function ProcessFiscalRetryStatusToJSON(value?: ProcessFiscalRetryStatus | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'folioSeqId': value.folioSeqId,
        'isLastRetry': value.isLastRetry,
        'links': value.links === undefined ? undefined : ((value.links as Array<any>).map(InstanceLinkToJSON)),
        'warnings': value.warnings === undefined ? undefined : ((value.warnings as Array<any>).map(WarningTypeToJSON)),
    };
}


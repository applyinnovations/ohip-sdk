/* tslint:disable */
/* eslint-disable */
/**
 * OPERA Cloud Cashiering API
 * APIs to cater for Cashiering related functionality in OPERA Cloud.<br /><br /> Cashiering provides access to a guest folio, posting journals, receipt histories, currency calculations, credit card settlements, and check a guest out.<br /><br /> Compatible with OPERA Cloud release 22.3.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2023 Oracle and/or its affiliates.</p>
 *
 * The version of the OpenAPI document: 22.3
 * Contact: hospitality_apis_ww_grp@oracle.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import type { CashierClosureType } from './CashierClosureType';
import {
    CashierClosureTypeFromJSON,
    CashierClosureTypeFromJSONTyped,
    CashierClosureTypeToJSON,
} from './CashierClosureType';
import type { InstanceLink } from './InstanceLink';
import {
    InstanceLinkFromJSON,
    InstanceLinkFromJSONTyped,
    InstanceLinkToJSON,
} from './InstanceLink';
import type { WarningType } from './WarningType';
import {
    WarningTypeFromJSON,
    WarningTypeFromJSONTyped,
    WarningTypeToJSON,
} from './WarningType';

/**
 * 
 * @export
 * @interface PutCashierRequest
 */
export interface PutCashierRequest {
    /**
     * 
     * @type {CashierClosureType}
     * @memberof PutCashierRequest
     */
    criteria?: CashierClosureType;
    /**
     * The Cashier Lock Handle to pass along with operation which required cashier to be locked.
     * @type {number}
     * @memberof PutCashierRequest
     */
    cashierLockHandle?: number;
    /**
     * Used for codes in the OPERA Code tables. Possible values of this pattern are 1, 101, 101.EQP, or 101.EQP.X.
     * @type {string}
     * @memberof PutCashierRequest
     */
    hotelId?: string;
    /**
     * 
     * @type {Array<InstanceLink>}
     * @memberof PutCashierRequest
     */
    links?: Array<InstanceLink>;
    /**
     * Used in conjunction with the Success element to define a business error.
     * @type {Array<WarningType>}
     * @memberof PutCashierRequest
     */
    warnings?: Array<WarningType>;
}

/**
 * Check if a given object implements the PutCashierRequest interface.
 */
export function instanceOfPutCashierRequest(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function PutCashierRequestFromJSON(json: any): PutCashierRequest {
    return PutCashierRequestFromJSONTyped(json, false);
}

export function PutCashierRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): PutCashierRequest {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'criteria': !exists(json, 'criteria') ? undefined : CashierClosureTypeFromJSON(json['criteria']),
        'cashierLockHandle': !exists(json, 'cashierLockHandle') ? undefined : json['cashierLockHandle'],
        'hotelId': !exists(json, 'hotelId') ? undefined : json['hotelId'],
        'links': !exists(json, 'links') ? undefined : ((json['links'] as Array<any>).map(InstanceLinkFromJSON)),
        'warnings': !exists(json, 'warnings') ? undefined : ((json['warnings'] as Array<any>).map(WarningTypeFromJSON)),
    };
}

export function PutCashierRequestToJSON(value?: PutCashierRequest | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'criteria': CashierClosureTypeToJSON(value.criteria),
        'cashierLockHandle': value.cashierLockHandle,
        'hotelId': value.hotelId,
        'links': value.links === undefined ? undefined : ((value.links as Array<any>).map(InstanceLinkToJSON)),
        'warnings': value.warnings === undefined ? undefined : ((value.warnings as Array<any>).map(WarningTypeToJSON)),
    };
}


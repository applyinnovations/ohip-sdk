"use strict";
/* tslint:disable */
/* eslint-disable */
/**
 * OPERA Cloud Block Reservation Asynchronous API
 * APIs to cater Block Reservation related asynchronous functionality in OPERA.<br /><br /> Compatible with OPERA Cloud release 22.4.0.0.<br /><br /><p> This document and all content within is available under the Universal Permissive License v 1.0 (https://oss.oracle.com/licenses/upl). Copyright (c) 2020, 2022 Oracle and/or its affiliates.</p>
 *
 * The version of the OpenAPI document: 22.4.0.0
 * Contact: hospitality_apis_ww_grp@oracle.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.BlockPotenitalRevenueTypeToJSON = exports.BlockPotenitalRevenueTypeFromJSONTyped = exports.BlockPotenitalRevenueTypeFromJSON = exports.instanceOfBlockPotenitalRevenueType = void 0;
const runtime_1 = require("../runtime");
/**
 * Check if a given object implements the BlockPotenitalRevenueType interface.
 */
function instanceOfBlockPotenitalRevenueType(value) {
    let isInstance = true;
    return isInstance;
}
exports.instanceOfBlockPotenitalRevenueType = instanceOfBlockPotenitalRevenueType;
function BlockPotenitalRevenueTypeFromJSON(json) {
    return BlockPotenitalRevenueTypeFromJSONTyped(json, false);
}
exports.BlockPotenitalRevenueTypeFromJSON = BlockPotenitalRevenueTypeFromJSON;
function BlockPotenitalRevenueTypeFromJSONTyped(json, ignoreDiscriminator) {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        'roomRevenue': !(0, runtime_1.exists)(json, 'roomRevenue') ? undefined : json['roomRevenue'],
        'foodRevenue': !(0, runtime_1.exists)(json, 'foodRevenue') ? undefined : json['foodRevenue'],
        'otherRevenue': !(0, runtime_1.exists)(json, 'otherRevenue') ? undefined : json['otherRevenue'],
        'nonRevenue': !(0, runtime_1.exists)(json, 'nonRevenue') ? undefined : json['nonRevenue'],
        'totalRevenue': !(0, runtime_1.exists)(json, 'totalRevenue') ? undefined : json['totalRevenue'],
        'roomRevenueTax': !(0, runtime_1.exists)(json, 'roomRevenueTax') ? undefined : json['roomRevenueTax'],
        'foodRevenueTax': !(0, runtime_1.exists)(json, 'foodRevenueTax') ? undefined : json['foodRevenueTax'],
        'otherRevenueTax': !(0, runtime_1.exists)(json, 'otherRevenueTax') ? undefined : json['otherRevenueTax'],
        'nonRevenueTax': !(0, runtime_1.exists)(json, 'nonRevenueTax') ? undefined : json['nonRevenueTax'],
        'totalRevenueTax': !(0, runtime_1.exists)(json, 'totalRevenueTax') ? undefined : json['totalRevenueTax'],
        'currency': !(0, runtime_1.exists)(json, 'currency') ? undefined : json['currency'],
    };
}
exports.BlockPotenitalRevenueTypeFromJSONTyped = BlockPotenitalRevenueTypeFromJSONTyped;
function BlockPotenitalRevenueTypeToJSON(value) {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        'roomRevenue': value.roomRevenue,
        'foodRevenue': value.foodRevenue,
        'otherRevenue': value.otherRevenue,
        'nonRevenue': value.nonRevenue,
        'totalRevenue': value.totalRevenue,
        'roomRevenueTax': value.roomRevenueTax,
        'foodRevenueTax': value.foodRevenueTax,
        'otherRevenueTax': value.otherRevenueTax,
        'nonRevenueTax': value.nonRevenueTax,
        'totalRevenueTax': value.totalRevenueTax,
        'currency': value.currency,
    };
}
exports.BlockPotenitalRevenueTypeToJSON = BlockPotenitalRevenueTypeToJSON;
